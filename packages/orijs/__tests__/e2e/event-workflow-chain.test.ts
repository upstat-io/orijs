...
function getCapturesForOrder(orderId: string): {
  correlationIds: CapturedIds;
  traceIds: CapturedIds;
} {
...
class OrderProcessingWorkflowConsumer implements IWorkflowConsumer<
  OrderWorkflowData,
  OrderWorkflowResult
> {
  public onComplete = async (
    ctx: WorkflowContext<OrderWorkflowData>,
    ...
    // Step 1: Validate
    captures.correlationIds.workflowStep1 = ctx.meta?.correlationId as
      string | undefined;
    captures.traceIds.workflowStep1 = ctx.meta?.traceId as string | undefined;

    // Step 2: Process
    captures.correlationIds.workflowStep2 = ctx.meta?.correlationId as
      string | undefined;
...
class OrderCreatedEventConsumer implements IEventConsumer<
  OrderCreatedPayload,
...
> {
  ...
  public onEvent = async (
    ctx: EventContext<OrderCreatedPayload>,
    ...
    const captures = getCapturesForOrder(ctx.data.orderId);
    captures.correlationIds.eventHandler = meta.correlationId as
      string | undefined;
...
class OrderController implements OriController {
  ...
  private createOrder = async (ctx: RequestContext) => {
    ...
    captures.correlationIds.controller = meta.correlationId as
      string | undefined;
...
describe("E2E: HTTP → Event → Workflow Chain", () => {
  ...
  describe("HTTP → Workflow chain (direct)", () => {
    ...
    it("should propagate traceId through workflow steps", async () => {
      ...
      expect(traceCaptures.traceIds.workflowStep1).toBe(
        traceCaptures.traceIds.controller,
      );
      expect(traceCaptures.traceIds.workflowStep2).toBe(
        traceCaptures.traceIds.controller,
      );
    });
    ...
    it("should generate unique correlationId when not provided in header", async () => {
      ...
      expect(captures.correlationIds.workflowStep1).toBe(
        captures.correlationIds.controller,
      );
      expect(captures.correlationIds.workflowStep2).toBe(
        captures.correlationIds.controller,
      );
    });
  });
  ...
  describe("context propagation verification", () => {
    it("should include correlationId in workflow result metadata", async () => {
      ...
      expect(result.workflowResult.propagatedRequestIds).toContain(
        correlationId,
      );
    });
    ...
    it("should maintain separate correlationIds for concurrent requests", async () => {
      ...
      expect(result1.workflowResult.propagatedRequestIds).toEqual([
        correlationId1,
        correlationId1,
      ]);
      ...
      expect(result2.workflowResult.propagatedRequestIds).toEqual([
        correlationId2,
        correlationId2,
      ]);
    });
  ...
  describe("error scenarios", () => {
    it("should propagate correlationId even when workflow step fails", async () => {
      ...
      const response = await fetch(
        `http://localhost:${testPort}/api/orders-fail`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
