...
describe("MapResult fluent API", () => {
  ...
  describe(".mergeWhen()", () => {
    ...
    it("should be chainable", () => {
      ...
      const result = UserMapper.map(row)
        .mergeWhen(true, extra1)
        .mergeWhen(true, extra2)
        .value();
  ...
  describe("combined usage", () => {
    ...
    it("should handle the common pattern: map().mergeWhen(!!extra, extra).default(null)", () => {
      ...
      const withExtra = UserMapper.map(row)
      ...
      const withoutExtra = UserMapper.map(row)
        .mergeWhen(!!noExtra, noExtra)
        .default(null);
      ...
      // Null row
      const nullRow = UserMapper.map(null)
        .mergeWhen(!!extra, extra)
        .default(null);
