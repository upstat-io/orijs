...
describe("Mapper dynamic prefix", () => {
  ...
  describe("basic prefix usage", () => {
    ...
    test("should throw MapperError when required prefixed columns are missing", () => {
      ...
      expect(() => StepMapper.map(row, { prefix: "step_" })).toThrow(
  ...
  describe("prefix with mapMany", () => {
    ...
    test("should throw when any row has missing required fields", () => {
      ...
      expect(() => StepMapper.mapMany(rows, { prefix: "item_" })).toThrow(
        "cannot coerce null/undefined",
      );
    });
  });
  ...
  describe("prefix with field renames", () => {
    ...
    const UserMapper = Mapper.for<UserRenamed>(TablesWithRename.User)
      .field("id")
      .as("oderId")
      .build();
  ...
  describe("real-world use case: automation steps", () => {
    ...
    const AutomationStepMapper = Mapper.for<AutomationStep>(
      AutomationTables.AutomationStep,
    ).build();
    ...
    test("should map automation steps with dynamic prefix", () => {
      ...
      const step1 = AutomationStepMapper.map(row, {
        prefix: "step_1_",
      }).value();
      const step2 = AutomationStepMapper.map(row, {
        prefix: "step_2_",
      }).value();
    ...
    test("should allow iterating with dynamic prefixes", () => {
      ...
      for (let i = 0; i < stepCount; i++) {
        const step = AutomationStepMapper.map(row, {
          prefix: `step_${i}_`,
        }).value();
