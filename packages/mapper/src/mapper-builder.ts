...
export class MapperBuilder<T> implements IMapperBuilder<T> {
   ...
   * @internal
   */
  private validatePropertyNotMapped(
    propertyName: string,
    source: string,
  ): void {
  ...
  public json<J>(
    column: string,
    factory?: (raw: unknown) => J | null | undefined,
  ): IJsonBuilder<T, J> {
  ...
  public col<C>(
    propertyName: string,
    columnOrCompute?:
      string | ((row: Record<string, unknown>) => C | null | undefined),
  ): IColBuilder<T, C> {
  ...
  public transform<K extends keyof T>(
    propertyName: K,
    fn: (value: T[K]) => T[K],
  ): IMapperBuilder<T> {
 ...
 */
class JsonBuilder<T, J>
  extends ChainedBuilder<T>
  implements IJsonBuilder<T, J>
{
