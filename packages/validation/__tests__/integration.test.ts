...
describe("Validation Integration", () => {
    ...
    test("should validate UUID params", async () => {
      ...
      class UserController implements OriController {
        configure(r: RouteBuilder) {
          r.get(
            "/:id",
            (ctx: RequestContext) => Response.json({ id: ctx.params.id }),
            {
      ...
      const invalidRes = await fetch(
        `http://localhost:${port}/users/not-a-uuid`,
      );
      expect(invalidRes.status).toBe(422);
      const errorData = (await invalidRes.json()) as {
        error: string;
        errors: unknown[];
      };
    ...
    test("should validate pagination query params", async () => {
      ...
      class ListController implements OriController {
        configure(r: RouteBuilder) {
          r.get(
            "/",
            (ctx: RequestContext) => Response.json({ query: ctx.query }),
            {
      ...
      // Valid pagination
      const validRes = await fetch(
        `http://localhost:${port}/items?page=1&limit=10`,
      );
      expect(validRes.status).toBe(200);
    ...
    test("should validate search query params", async () => {
      ...
      class SearchController implements OriController {
        configure(r: RouteBuilder) {
          r.get(
            "/",
            (ctx: RequestContext) => Response.json({ q: ctx.query.q }),
            {
  ...
  describe("Combined validation", () => {
    test("should validate params, query, and body together", async () => {
      ...
      // Invalid param
      const invalidParamRes = await fetch(
        `http://localhost:${port}/items/not-uuid?q=test`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name: "Test Item" }),
  ...
  describe("No validation", () => {
    test("should skip validation when no schema provided", async () => {
      ...
      class SimpleController implements OriController {
        configure(r: RouteBuilder) {
          r.get("/:anything", (ctx: RequestContext) =>
            Response.json({ param: ctx.params.anything }),
          );
      ...
      const paramRes = await fetch(
        `http://localhost:${port}/api/anything-goes`,
      );
  ...
  describe("Custom validators", () => {
    ...
    test("should validate with async custom function", async () => {
      ...
      const errorData = (await takenRes.json()) as {
