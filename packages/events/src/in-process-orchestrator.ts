...
export class InProcessEventProvider implements EventProvider {
  ...
  private idempotencyCleanupInterval: ReturnType<typeof setInterval> | null =
    null;
  ...
  private scheduleDelivery<TReturn>(
    ...
    if (key) {
      this.pendingDelayedByKey.set(key, {
        timeout,
        subscription: subscription as EventSubscription<unknown>,
      });
    }
  }
}
