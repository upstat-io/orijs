...
function createMockProvider(
  subscribeDelay = 0,
): EventProvider & { subscribeCalls: string[] } {
  const subscribeCalls: string[] = [];
  return {
    subscribeCalls,
    subscribe<TPayload, TReturn>(
      eventName: string,
      _handler: EventHandlerFn<TPayload, TReturn>,
    ): Promise<void> | void {
...
describe("createEventSystem subscription tracking", () => {
  ...
  test("should await all subscriptions during start()", async () => {
    ...
    const startPromise = events.start().then(() => {
      startResolved = true;
    });
  ...
  test("should capture post-start registration failures via onRegistrationError", async () => {
    ...
    provider.subscribe = <TPayload, TReturn>(
      _eventName: string,
      _handler: EventHandlerFn<TPayload, TReturn>,
    ) => {
