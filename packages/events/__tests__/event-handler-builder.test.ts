...
describe("EventHandlerBuilder", () => {
  ...
  describe("on()", () => {
    ...
    it("should register multiple handlers for different events", () => {
      ...
      expect(registrations.map((r) => r.eventName)).toEqual([
        "event.a",
        "event.b",
        "event.c",
      ]);
  ...
  describe("registerWith()", () => {
    ...
    it("should return handler result through wrapped handler", async () => {
      ...
      builder.on<{ input: string }, { output: string }>(
        "test.event",
        async (ctx) => {
      ...
      const mockProvider = {
        subscribe: (
          _eventName: string,
          handler: EventHandlerFn<unknown, unknown>,
        ) => {
          capturedHandler = handler;
        },
        emit: () => ({}) as EventSubscription<any>,
      ...
      await builder.registerWith(
        mockProvider,
        (() => ({}) as EventSubscription<any>) as any,
      );
  ...
  describe("EventHandlerClass pattern", () => {
    it("should work with handler classes", () => {
      ...
      class OrderHandler {
        ...
        private handlePlaced = async (
          _ctx: EventContext<{ orderId: string }>,
        ) => {
          return { processed: true };
        };

        private handleShipped = async (
          _ctx: EventContext<{ trackingNumber: string }>,
        ) => {
          return { shipped: true };
        };
      }
  ...
  describe("edge cases", () => {
    ...
    it("should handle chained emit with delay option", async () => {
      ...
      expect(emitCalls[0]![2]).toEqual({
        delay: 1000,
        causationId: "corr-456",
      });
    });
