...
describe("TestEventProvider", () => {
  ...
  describe("async delivery verification", () => {
    ...
    it("should support async/await on emit result", async () => {
      provider.subscribe<{ name: string }, { greeting: string }>(
        "greet",
        async (msg) => {
      ...
      const result = await provider.emit<{ greeting: string }>(
        "greet",
        { name: "World" },
        {},
      );
    ...
    it("should respect emit delay on top of processing delay", async () => {
      ...
      const result = await provider.emit<{ done: boolean }>(
        "delayed",
        {},
        {},
        { delay: 20 },
  ...
  describe("request-response pattern", () => {
    it("should return handler result via await", async () => {
      ...
      provider.subscribe<CheckPayload, CheckResult>(
        "monitor.check",
        async (_msg) => {
      ...
      const result = await provider.emit<CheckResult>(
        "monitor.check",
    ...
    it("should work with .subscribe() callback", async () => {
      ...
      provider
        .emit<{ value: number }>("callback.test", {}, {})
        .subscribe((result) => {
          received = result;
        });
  ...
  describe("cancel", () => {
    it("should cancel a delayed event by key", async () => {
      ...
      provider.emit(
        ...
        {},
        {},
        { delay: 100, idempotencyKey: "cancel-me" },
      );
