...
describe("InProcessEventProvider", () => {
  ...
  describe("delayed events", () => {
    ...
    it("should cancel a delayed event by key", async () => {
      ...
      const meta: PropagationMeta = {};
      provider.emit("delayed.event", {}, meta, {
        delay: 100,
  ...
  describe("idempotency", () => {
    it("should deduplicate events with same idempotencyKey", async () => {
      ...
      const subscription1 = provider.emit("test.event", { value: 1 }, meta, {
        idempotencyKey,
      });
      const subscription2 = provider.emit("test.event", { value: 2 }, meta, {
        idempotencyKey,
      });
    ...
    it("should resolve duplicate emission with undefined", async () => {
      ...
      const duplicateSubscription = provider.emit("test.event", {}, meta, {
        idempotencyKey,
      });
