...
describe("EventSystem", () => {
  ...
  describe("emit", () => {
    ...
    it("should throw for unknown events", () => {
      expect(() => {
        ...
        (eventSystem.emit as (n: string, p: unknown) => unknown)(
          "unknown.event",
          {},
        );
    ...
    it("should return EventSubscription for request-response", async () => {
      eventSystem.onEvent<OrderPlacedPayload, OrderResult>(
        "order.placed",
        async () => {
  ...
  describe("onEvent", () => {
    ...
    it("should throw for unknown events", () => {
      expect(() => {
        // Cast to any to test runtime validation of invalid event name
        (eventSystem.onEvent as (n: string, h: () => Promise<void>) => void)(
          "unknown.event",
          async () => {},
        );
      }).toThrow("Unknown event: unknown.event");
...
describe("EventSystem Integration", () => {
  ...
  it("should support async/await for request-response", async () => {
    ...
    system.onEvent<{ amount: number }, { transactionId: string }>(
      "payment.initiated",
      async (ctx) => {
    ...
    const result = await system.emit<{ transactionId: string }>(
      "payment.initiated",
      { amount: 100 },
    );
  ...
  it("should support chained event emission from handler", async () => {
    ...
    system.onEvent<{ paymentId: string; status: string }>(
      "payment.completed",
      async (ctx) => {
    ...
    expect(completedPayments[0]).toEqual({
      paymentId: "PAY-001",
      status: "success",
    });
...
describe("EventSystem Context Propagation", () => {
  ...
  it("should propagate trace context from AsyncLocalStorage to event metadata", async () => {
    ...
    await runWithContext(
      { log: Logger.console(), correlationId: "req-123", trace },
      async () => {
  ...
  it("should propagate only correlationId when trace context is missing", async () => {
    ...
    await runWithContext(
      { log: Logger.console(), correlationId: "req-456" },
      async () => {
        system.emit("test.event", { value: 1 });
        await new Promise((resolve) => setTimeout(resolve, 10));
      },
  ...
  it("should create child span for each event emission", async () => {
    ...
    await runWithContext(
      { log: Logger.console(), correlationId: "req-789", trace },
      async () => {
        system.emit("test.event", { value: 1 });
