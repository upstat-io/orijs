...
describe("ScheduledEventManager", () => {
  describe("schedule creation", () => {
    it("should create job scheduler with cron pattern", async () => {
      ...
      const { ScheduledEventManager } =
        await import("../../src/events/scheduled-event-manager.ts");
  ...
  describe("schedule listing", () => {
    it("should return all active schedules for an event type", async () => {
      ...
      const intervalSchedule = schedules.find(
        (schedule) => schedule.scheduleId === "check-1",
      );
      const cronSchedule = schedules.find(
        (schedule) => schedule.scheduleId === "check-2",
      );
  ...
  describe("lifecycle", () => {
    it("should close all queues on stop", async () => {
      ...
      await manager.schedule("event.a", {
        scheduleId: "a",
        every: 1000,
        payload: {},
      });
      await manager.schedule("event.b", {
        scheduleId: "b",
        every: 1000,
        payload: {},
      });
