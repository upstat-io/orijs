...
describe("BullMQEventProvider", () => {
  describe("emit", () => {
    it("should establish completion ownership before enqueueing a request-response job", async () => {
      ...
      expect(mockCompletionTracker.waitUntilReady).toHaveBeenCalledWith(
        "event.fast.void",
      );
    ...
    it("should attach to a retained completed request-response job", async () => {
      ...
      const mockQueueManager = {
        getQueueName: mock((eventName: string) => `event.${eventName}`),
        addJob: mock(
          (_eventName: string, _data: unknown, options: { jobId: string }) =>
            Promise.resolve({
              id: options.jobId,
    ...
    it("should register completion tracking for request-response", async () => {
      ...
      const subscription = provider.emit<{ processed: boolean }>(
        "monitor.check",
        { monitorId: "123" },
        {},
      );
    ...
    it("should propagate job creation error to subscription catch callback", async () => {
      ...
      const mockCompletionTracker = {
        ...
        mapJobId: mock(() => {}),
        fail: mock(
          (_queueName: string, _correlationId: string, error: Error) => {
  ...
  describe("TTL (configureEvent)", () => {
    ...
    it("should clear sweep interval on stop", async () => {
      ...
      expect(mockQueueManager.cleanJobs.mock.calls.length).toBe(
        callCountAfterStop,
      );
    });
  });
  ...
  describe("subscribe", () => {
    ...
    it("should wrap handler to extract EventMessage from job data", async () => {
      ...
      let capturedCompletedHandler:
        ((jobId: string, result: any) => void) | null = null;

      const mockQueueManager = {
        addJob: mock(() => Promise.resolve({ id: "job-123" })),
        getQueueName: mock((eventName: string) => `event.${eventName}`),
        registerWorker: mock(
          (_eventName: string, handler: any, onCompleted: any) => {
  ...
  describe("scheduleEvent", () => {
    it("should delegate to ScheduledEventManager", async () => {
      ...
      expect(mockScheduledManager.schedule).toHaveBeenCalledWith(
        "monitor.check",
        {
          scheduleId: "hourly-check",
          cron: "0 * * * *",
    ...
    it("should support unschedule", async () => {
      ...
      expect(mockScheduledManager.unschedule).toHaveBeenCalledWith(
        "monitor.check",
        "hourly-check",
      );
    });
