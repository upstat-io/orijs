...
describe("BullMQ Event Provider E2E Scenarios", () => {
  ...
  describe("Scenario: Cascading Events", () => {
    it("should handle event chain: monitor.failed -> alert.triggered -> notification.sent", async () => {
      ...
      await provider.subscribe<MonitorFailedPayload>(
      ...
      await provider.subscribe<AlertTriggeredPayload>(
        eventName("cascade.alert.triggered"),
        async (msg) => {
      ...
      await provider.subscribe<NotificationSentPayload>(
        ...
        async (msg) => {
          eventLog.push(
            `notification.sent:${msg.payload.alertId}:${msg.payload.channel}`,
          );
        },
      );
      ...
      provider.emit(
        eventName("cascade.monitor.failed"),
        {
          monitorId: "mon-cascade",
          errorMessage: "Connection timeout",
        } as MonitorFailedPayload,
        meta,
      );
    ...
    it("should preserve causation chain through cascading events", async () => {
      ...
      await provider.subscribe<MonitorFailedPayload>(
        eventName("chain.monitor.failed"),
        async (msg) => {
          causationChain.push({
      ...
      provider.emit(
        eventName("chain.monitor.failed"),
        { monitorId: "mon1", errorMessage: "fail" },
        {},
      );
      ...
      expect(causationChain[1]?.causationId).toBe(
        causationChain[0]?.correlationId,
      );
    });
  });
  ...
  describe("Scenario: Delayed Event with Context", () => {
    it("should process delayed event with full context preservation", async () => {
      ...
      await provider.subscribe<MonitorCheckPayload>(
      ...
      provider.emit(
        eventName("delayed.monitor.check"),
        {
          monitorId: "mon-delayed",
          url: "https://example.com",
        } as MonitorCheckPayload,
        meta,
        { delay: 100 },
      );
  ...
  describe("Scenario: Multi-Instance Concurrent Processing", () => {
    it("should process 20 events across 3 app instances without message loss", async () => {
      ...
      for (const event of processedEvents) {
        instanceCounts.set(
          event.instanceId,
          (instanceCounts.get(event.instanceId) ?? 0) + 1,
        );
      }
    ...
    it("should handle 3 event types across 3 instances without cross-contamination", async () => {
      ...
      for (let i = 0; i < instanceCount; i++) {
        ...
        await worker.subscribe<
          { id: number },
          { type: string; id: number; instanceId: number }
        >(eventName("mixed.multi.typeA"), async (msg) => {
        ...
        await worker.subscribe<
          { id: number },
          { type: string; id: number; instanceId: number }
        >(eventName("mixed.multi.typeB"), async (msg) => {
        ...
        await worker.subscribe<
          { id: number },
          { type: string; id: number; instanceId: number }
        >(eventName("mixed.multi.typeC"), async (msg) => {
      ...
      expect(typeAResults.every((r) => r.id >= 0 && r.id < eventsPerType)).toBe(
        true,
      );
      expect(
        typeBResults.every((r) => r.id >= 1000 && r.id < 1000 + eventsPerType),
      ).toBe(true);
      expect(
        typeCResults.every((r) => r.id >= 2000 && r.id < 2000 + eventsPerType),
      ).toBe(true);
      ...
      expect(
        new Set(typeAResults.map((r) => r.instanceId)).size,
      ).toBeGreaterThan(1);
      expect(
        new Set(typeBResults.map((r) => r.instanceId)).size,
      ).toBeGreaterThan(1);
      expect(
        new Set(typeCResults.map((r) => r.instanceId)).size,
      ).toBeGreaterThan(1);
    }, 8000);
    ...
    it("should preserve context across all instances", async () => {
      ...
      for (let i = 0; i < instanceCount; i++) {
        ...
        await worker.subscribe<{ id: number }>(
      ...
      expect(
        new Set(contextResults.map((r) => r.instanceId)).size,
      ).toBeGreaterThan(1);
    }, 8000);
  });
});
