...
function buildMocks(addJob?: () => Promise<{ id: string }>) {
  const queueManager = {
    ...
    registerWorker: mock(
      (_eventName: string, _handler: (job: unknown) => Promise<unknown>) =>
        Promise.resolve(),
    ),
    getQueue: mock(() => ({
...
async function buildProvider(mocks: ReturnType<typeof buildMocks>) {
  const { BullMQEventProvider } =
    await import("../../src/events/bullmq-event-provider.ts");
...
describe("fire-and-forget emit", () => {
  ...
  it("should skip re-enqueue when a durable completion receipt exists", async () => {
    const mocks = buildMocks();
    mocks.queueManager.hasCompletionReceipt.mockImplementation(() =>
    ...
    await provider.emit(
      ...
      {},
      {},
      { expectsResult: false, idempotencyKey: "delivery-key" },
    );

    expect(mocks.queueManager.hasCompletionReceipt).toHaveBeenCalledWith(
      "email.sent",
      "delivery-key",
    );
  ...
  it("should prepare and finalize receipt retirement through the provider boundary", async () => {
    ...
    await provider.prepareIdempotencyKeyRetirement(
      "email.sent",
      "delivery-key",
    );
    await provider.finalizeIdempotencyKeyRetirement(
      "email.sent",
      "delivery-key",
    );

    expect(
      mocks.queueManager.prepareCompletionReceiptRetirement,
    ).toHaveBeenCalledWith("email.sent", "delivery-key");
    expect(
      mocks.queueManager.finalizeCompletionReceiptRetirement,
    ).toHaveBeenCalledWith("email.sent", "delivery-key");
  });
  ...
  it("should expose durable completion receipts for outbox reconciliation", async () => {
    ...
    expect(
      await provider.hasSuccessfulIdempotencyKeyCompletionReceipt(
        "escalation.stopped",
        "delivery-key",
      ),
    ).toBe(true);
    expect(
      mocks.queueManager.hasSuccessfulCompletionReceipt,
    ).toHaveBeenCalledWith("escalation.stopped", "delivery-key");
  });
  ...
  it("should report whether a parent event job remains retained", async () => {
    ...
    expect(await provider.hasRetainedEvent("catalog.changed", "event-id")).toBe(
      false,
    );
    expect(mocks.queueManager.hasJob).toHaveBeenCalledWith(
      "catalog.changed",
      "event-id",
    );
  });
  ...
  it("should use the durable event identity as the default BullMQ job ID", async () => {
    ...
    const [, message, options] = mocks.queueManager.addJob.mock
      .calls[0] as unknown as [string, { eventId: string }, { jobId: string }];
  ...
  it("should record the durable receipt only after the child consumer succeeds", async () => {
    ...
    let workerHandler:
      ...
      | undefined;
    mocks.queueManager.registerWorker.mockImplementation(
      (_eventName, handler) => {
    ...
    const provider = await buildProvider(mocks);
    await provider.subscribe("email.sent", () =>
    ...
    expect(mocks.queueManager.recordCompletionReceipt).toHaveBeenCalledWith(
      "email.sent",
      "delivery-key",
    );
  });
  ...
  it("should surface the enqueue failure when the job cannot be queued", async () => {
    ...
    await expect(Promise.resolve(subscription)).rejects.toThrow(
      "queue unavailable",
    );
