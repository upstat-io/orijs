...
> {
  ...
  public onComplete = async (
    ctx: WorkflowContext<ContextCapturingData>,
  ): Promise<ContextCapturingResult> => {
    // Access the propagation metadata from the workflow context
    const meta =
      (
        ctx.log as { propagationMeta?: () => Record<string, unknown> }
      )?.propagationMeta?.() ??
      ctx.meta ??
      {};
 ...
 */
class MultiStepWorkflowConsumer implements IWorkflowConsumer<
  MultiStepData,
  MultiStepResult
> {
  // Phase 1: No-op
  ...
  public onComplete = async (
    ctx: WorkflowContext<MultiStepData>,
...
describe("cross-service context propagation", () => {
  ...
  it("should propagate auth guard metadata through workflow execution", async () => {
    ...
    class Instance1Controller implements OriController {
      ...
      private executeWorkflow = async (ctx: RequestContext) => {
        ...
        const handle = await ctx.app.workflows.execute(
          ContextCapturingWorkflowDef,
          {
            value: body.value,
          },
        );
    ...
    const response = await fetch(
      `http://localhost:${currentPort1}/api/execute`,
      {
        method: "POST",
        headers: {
  ...
  it("should propagate context through BullMQ distributed workflow execution", async () => {
    ...
    const response = await fetch(
      `http://localhost:${currentPort}/api/execute`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    expect(result.capturedContext.correlationId).toBe(
      "req-bullmq-distributed-123",
    );
  ...
  it("should maintain trace chain across multiple workflow steps", async () => {
    class TraceController implements OriController {
      ...
      private runMultiStep = async (ctx: RequestContext) => {
        ...
        const handle = await ctx.app.workflows.execute(MultiStepWorkflowDef, {
          input: body.input,
        });
        return Response.json(await handle.result());
      };
    ...
    const response = await fetch(
      `http://localhost:${currentPort}/api/multi-step`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
