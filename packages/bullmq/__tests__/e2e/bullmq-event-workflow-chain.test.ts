...
class OrderController implements OriController {
  ...
  private createOrderDirect = async (ctx: RequestContext) => {
    ...
    const capturedForRequest = correlationId
      ? getCapturedContext(correlationId)
      : capturedContext;
...
describe("E2E: BullMQ Event → Workflow Chain", () => {
  ...
  describe("HTTP → Workflow chain (direct, BullMQ)", () => {
    it("should propagate correlationId from HTTP through BullMQ workflow steps", async () => {
      ...
      const response = await fetch(
        `http://localhost:${testPort}/api/orders-direct`,
        {
          method: "POST",
          headers: {
    ...
    it("should propagate traceId through BullMQ workflow steps", async () => {
      ...
      expect(capturedContext.workflowStep1?.traceId).toBe(
        capturedContext.controller?.traceId,
      );
      expect(capturedContext.workflowStep2?.traceId).toBe(
        capturedContext.controller?.traceId,
      );
    }, 15000);
  });
  ...
  describe("context preservation in all steps", () => {
    it("should propagate context through multi-step workflow", async () => {
      ...
      class TwoStepWorkflowConsumer implements IWorkflowConsumer<
        TwoStepData,
        TwoStepResult
      > {
        public onComplete = async (
          ctx: WorkflowContext<TwoStepData>,
      ...
      const response = await fetch(
        `http://localhost:${testPort}/api/multi-step`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      expect(captured.workflowStep1?.traceId).toBe(
        captured.controller?.traceId,
      );
      expect(captured.workflowStep2?.traceId).toBe(
        captured.controller?.traceId,
      );
    }, 15000);
  });
      ...
      class StrictController implements OriController {
        ...
        private execute = async (ctx: RequestContext) => {
          ...
          try {
            ...
            await this.appContext.workflows.execute(
              StrictWorkflowDef,
              body as any,
            );
  ...
  describe("idempotency and exactly-once delivery", () => {
    it("should not process duplicate workflow executions", async () => {
      ...
      class CountingWorkflowConsumer implements IWorkflowConsumer<
        CountingData,
        CountingResult
      > {
