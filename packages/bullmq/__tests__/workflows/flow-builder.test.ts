  ...
  describe("buildFlow", () => {
    it("bounds completed and failed retention for parent and step jobs", () => {
      const flow = new FlowBuilder({
        ...
        [{ type: "sequential", definitions: [createStep("step1")] }],
        {},
      );

      expect(flow.opts?.removeOnComplete).toEqual({
        age: 604800,
        count: 10_000,
      });
      expect(flow.opts?.removeOnFail).toEqual({ age: 2592000, count: 10_000 });
      expect(flow.children?.[0]?.opts?.removeOnComplete).toEqual({
        age: 604800,
        count: 10_000,
      });
    });
    ...
    it("should build parallel steps as single parallel group job", () => {
      ...
      const stepGroups: StepGroup[] = [
        {
          type: "parallel",
          definitions: [
            createStep("notify-email"),
            createStep("notify-sms"),
            createStep("notify-slack"),
          ],
        },
      ];
      ...
      expect(flow.children).toHaveLength(1);
      const parallelJob = flow.children![0]!;
      expect(parallelJob.name).toBe(
        "__parallel__:notify-email,notify-sms,notify-slack",
      );
