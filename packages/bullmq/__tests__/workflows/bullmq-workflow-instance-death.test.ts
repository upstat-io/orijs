...
const createTrackedStepHandlers = () => ({
  ...
  step2: {
    execute: async (ctx: WorkflowContext<{ value: number }>) => {
      const prev =
        (ctx.results["step1"] as { computed: number })?.computed ??
        ctx.data.value;
      executionLog.log({
        instanceName: ctx.providerId ?? "unknown",
        workflowName: "TrackedWorkflow",
  ...
  step3: {
    execute: async (ctx: WorkflowContext<{ value: number }>) => {
      const prev =
        (ctx.results["step2"] as { computed: number })?.computed ??
        ctx.data.value;
      executionLog.log({
        instanceName: ctx.providerId ?? "unknown",
        workflowName: "TrackedWorkflow",
  ...
  describe("graceful shutdown", () => {
      ...
      async () => {
        harness.registerConsumer(
          ...
          async (_data, _meta, stepResults) => {
            const step3Result = stepResults?.["step3"] as
        ...
        console.log(
          `Graceful shutdown: backup executed ${backupEntries.length} steps`,
        );
    ...
    it(
      ...
      async () => {
        ...
        const handle = await worker1.execute(TrackedWorkflowDef, {
          value: 100,
        });
        ...
        console.log(
          `Mid-workflow stop: worker1=${w1Entries.length}, worker2=${w2Entries.length}`,
        );
      },
      TEST_TIMEOUTS.WORKFLOW_EXECUTION,
    );
    ...
    it(
      ...
      async () => {
        ...
        const replacementEntries =
          executionLog.getEntriesForInstance("replacement");
        console.log(
          `Replacement test: replacement executed ${replacementEntries.length} steps`,
        );
      },
      TEST_TIMEOUTS.WORKFLOW_EXECUTION,
    );
  ...
  describe("all workers stop", () => {
    it(
      ...
      async () => {
        ...
        await delay(
          TEST_TIMEOUTS.STALL_RECOVERY_WAIT +
        ...
        console.log(
          `Revival test: revival executed ${revivalEntries.length} steps`,
        );
      },
      TEST_TIMEOUTS.WORKFLOW_EXECUTION * 2,
    );
