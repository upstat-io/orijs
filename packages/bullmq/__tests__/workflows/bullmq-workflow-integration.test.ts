...
const sequentialStepGroups = [
  {
    ...
    definitions: [{ name: "double" }, { name: "add10" }],
  },
];
...
const createSequentialStepHandlers = () => ({
  ...
  add10: {
    execute: async (ctx: WorkflowContext<{ value: number }>) => {
      const doubleResult = ctx.results["double"] as
        { doubled: number } | undefined;
...
const parallelStepGroups = [
  {
    ...
    definitions: [{ name: "validate" }, { name: "enrich" }, { name: "index" }],
  },
];
...
const mixedStepGroups = [
  {
    type: "sequential" as const,
    definitions: [{ name: "parse" }, { name: "transform" }],
  },
  {
    type: "parallel" as const,
    definitions: [{ name: "notify-email" }, { name: "notify-slack" }],
  },
];
...
const stepResultsStepGroups = [
  {
    type: "sequential" as const,
    definitions: [{ name: "multiply" }, { name: "add" }, { name: "square" }],
  },
];
...
const createStepResultsStepHandlers = () => ({
  ...
  add: {
    execute: async (ctx: WorkflowContext<{ value: number }>) => {
      const multiplyResult = ctx.results["multiply"] as
...
const errorStepGroups = [
  {
    type: "sequential" as const,
    definitions: [{ name: "check" }, { name: "process" }],
  },
];
...
describe("BullMQWorkflowProvider Integration", () => {
    ...
    it("should execute steps in sequence and return result", async () => {
      provider.registerDefinitionConsumer(
        ...
        async (_data, _meta, stepResults) => {
          const add10Result = stepResults?.["add10"] as
      ...
      const handle = await provider.execute(SequentialWorkflowDef, {
        value: 5,
      });
  ...
  describe("parallel workflow", () => {
    it("should execute parallel steps concurrently", async () => {
      ...
      const handle = await provider.execute(ParallelWorkflowDef, {
        items: ["a", "b", "c"],
      });
  ...
  describe("mixed workflow", () => {
    it("should execute sequential steps then parallel steps", async () => {
      ...
      const handle = await provider.execute(MixedWorkflowDef, {
        input: "hello",
      });
  ...
  describe("step results accumulation", () => {
    it("should key step results by step name, not job key", async () => {
      provider.registerDefinitionConsumer(
        ...
        async (_data, _meta, stepResults) => {
          const squareResult = stepResults?.["square"] as
            { computed: number } | undefined;
      ...
      const handle = await provider.execute(StepResultsWorkflowDef, {
        value: 5,
      });
  ...
  describe("error handling", () => {
    it("should handle step errors gracefully", async () => {
      ...
      const handle = await provider.execute(ErrorWorkflowDef, {
        shouldFail: true,
      });
  ...
  describe("multiple workflows", () => {
    it("should support multiple registered workflows", async () => {
      ...
      const handle1 = await provider.execute(SequentialWorkflowDef, {
        value: 3,
      });
      const handle2 = await provider.execute(ParallelWorkflowDef, {
        items: ["x", "y"],
      });
    ...
    it("should execute multiple instances of same workflow concurrently", async () => {
      ...
      const results = await Promise.all(
        handles.map((h) =>
          withTimeout(h.result(), TEST_TIMEOUTS.CONCURRENT_WORKFLOWS),
        ),
      );
  ...
  describe("lifecycle", () => {
    ...
    it("should reject execute before start", async () => {
      ...
      await expect(
        provider.execute(SequentialWorkflowDef, { value: 1 }),
      ).rejects.toThrow("Provider not started");
    });
  ...
  describe("rollback context propagation", () => {
    it("should propagate meta context to rollback handlers", async () => {
      ...
      expect(rollbackCapturedContext.step1Rollback?.traceId).toBe(
        "test-trace-123",
      );
      expect(rollbackCapturedContext.step1Rollback?.correlationId).toBe(
        "test-request-456",
      );
    }, 15000);
  });
