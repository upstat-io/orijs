...
import {
  ...
  StepNotFoundError,
  createStepRegistry,
} from "../../src/workflows/step-registry.ts";
...
describe("StepRegistry", () => {
  ...
  describe("getStep", () => {
    ...
    it("should throw StepNotFoundError when workflow is unknown", () => {
      ...
      expect(() => registry.getStep("UnknownWorkflow", "step1")).toThrow(
        StepNotFoundError,
      );
    });
    ...
    it("should throw StepNotFoundError when step is unknown", () => {
      ...
      expect(() => registry.getStep("TestWorkflow", "unknownStep")).toThrow(
        StepNotFoundError,
      );
    });
  });
  ...
  describe("getRollback", () => {
    ...
    it("should return undefined when step is unknown", () => {
      ...
      expect(
        registry.getRollback("TestWorkflow", "unknownStep"),
      ).toBeUndefined();
    });
  });
});
...
describe("StepNotFoundError", () => {
  it("should have correct name and message", () => {
    ...
    expect(error.message).toBe(
      "Step 'myStep' not found for workflow 'MyWorkflow'",
    );
