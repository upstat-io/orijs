...
describe("BullMQ Workflow 2-Instance Distribution", () => {
  ...
  describe("rollback distribution", () => {
    it("should execute rollback on available instance when step fails", async () => {
      ...
      const instance1 = harness.getInstance("instance1");
      const handle = await instance1.execute(FailingTrackedWorkflowDef, {
        value: 5,
      });
      ...
      const step1Executions = executionLog
        .getEntriesForStep("step1")
        .filter((e) => e.action === "execute");
      const step1Rollbacks = executionLog
        .getEntriesForStep("step1")
      ...
      const step2Executions = executionLog
        .getEntriesForStep("step2")
        .filter((e) => e.action === "execute");
