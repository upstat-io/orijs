...
const simpleOnComplete = async (
  ...
  const processResult = stepResults?.["process"] as
 ...
 */
function getPrivateMap<K, V>(
  provider: BullMQWorkflowProvider,
  mapName: string,
): Map<K, V> {
...
describe("BullMQWorkflowProvider Timeout Handle Memory Leak", () => {
  ...
  describe("timeoutHandles cleanup", () => {
    it("should clean up timeout handle when workflow completes normally before timeout", async () => {
      ...
      const timeoutHandles = getPrivateMap<
        string,
        ReturnType<typeof setTimeout>
      >(provider, "timeoutHandles");
    ...
    it("should clean up timeout handle when workflow fails before timeout", async () => {
      ...
      // Execute a workflow
      const flowHandle = await provider.execute(SimpleWorkflowDef, {
        value: 42,
      });
    ...
    it("should clean up timeout handle synchronously in callback even if promise already settled", async () => {
      ...
      const timeoutHandles = getPrivateMap<
        ...
        ReturnType<typeof setTimeout>
      >(provider, "timeoutHandles");
      const localFlowStates = getPrivateMap<string, { status: string }>(
        provider,
        "localFlowStates",
      );
      ...
      const flowHandle2 = await provider.execute(SimpleWorkflowDef, {
        value: 99,
      });
