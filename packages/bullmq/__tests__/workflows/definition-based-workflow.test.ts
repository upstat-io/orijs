...
describe("Definition-Based Workflow Execution", () => {
  ...
  let mockQueueEventsInstances: Map<
    string,
    { on: ReturnType<typeof mock>; close: ReturnType<typeof mock> }
  >;
  ...
  beforeEach(() => {
    ...
    const MockQueueEventsClass = class {
      ...
      constructor(queueName: string) {
        mockQueueEventsInstances.set(queueName, {
          on: this.on,
          close: this.close,
        });
      }
    ...
    const options: BullMQWorkflowProviderOptions = {
      ...
      WorkerClass:
        MockWorkerClass as unknown as BullMQWorkflowProviderOptions["WorkerClass"],
      QueueEventsClass:
        MockQueueEventsClass as unknown as BullMQWorkflowProviderOptions["QueueEventsClass"],
    };
  ...
  describe("execute() with WorkflowDefinition", () => {
    ...
    it("should create QueueEvents for the workflow queue", async () => {
      ...
      expect(
        mockQueueEventsInstances.has("test-workflow.queue-events-workflow"),
      ).toBe(true);
    });
    ...
    it("should return a FlowHandle with id", async () => {
      ...
      const handle = await provider.execute(definition as any, {
        userId: "user-jkl",
      });
    ...
    it("should throw if provider not started", async () => {
      ...
      await expect(
        provider.execute(definition as any, { userId: "user-stu" }),
  ...
  describe("queue name format", () => {
    it("should handle workflow names with hyphens", async () => {
      ...
      expect(capturedJobs[0]!.queueName).toBe(
        "test-workflow.my-complex-workflow-name",
      );
    });
    ...
    it("should use custom queuePrefix", async () => {
      ...
      expect(capturedJobs[capturedJobs.length - 1]!.queueName).toBe(
        "custom-prefix.test",
      );
