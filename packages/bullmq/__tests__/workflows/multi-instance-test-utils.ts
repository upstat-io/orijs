    ...
    async waitForEntries(count: number, timeoutMs = 10000): Promise<void> {
      ...
      while (entries.length < count) {
        if (Date.now() - start > timeoutMs) {
          throw new Error(
            `Timeout waiting for ${count} entries, got ${entries.length}`,
          );
        }
...
export class MultiInstanceTestHarness {
  ...
  async startAll(): Promise<void> {
    await Promise.all(
      Array.from(this.instances.values()).map((instance) => instance.start()),
    );
  }
  ...
  async stopAll(): Promise<void> {
    await Promise.all(
      Array.from(this.instances.values()).map((instance) => instance.stop()),
    );
