      ...
      async () => {
        ...
        harness.registerConsumer(
          ...
          async (_data, _meta, stepResults) => {
            const stepResult = stepResults?.["compute"] as
        ...
        const handle = await instance1.execute(SimpleWorkflowDef, {
          value: 21,
        });
    ...
    it(
      ...
      async () => {
        ...
        harness.registerConsumer(
          "slow-workflow",
          async (_data, _meta, stepResults) => {
            const stepResult = stepResults?.["step3"] as
        ...
        expect(["running", "active", "waiting"]).toContain(
          statusFromReplacement,
        );
  ...
  describe("handle reconstruction", () => {
    it(
      ...
      async () => {
        ...
        const originalHandle = await first.execute(SimpleWorkflowDef, {
          value: 25,
        });
