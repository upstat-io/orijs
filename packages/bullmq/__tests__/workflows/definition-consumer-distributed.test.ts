...
describe("Definition-Based Consumer Distributed Workflow (E2E)", () => {
  ...
  describe("Basic Handler Flow (Current Implementation)", () => {
    it("should reconstruct structured rollback failure for a remote emitter", async () => {
      ...
      consumerProvider.registerDefinitionConsumer(
        ...
        stepGroups,
        {
          first: {
            execute: async () => "done",
            rollback: async () => Promise.reject(rollback),
          },
      ...
      expect((structured.primaryError as WorkflowStepError).stepName).toBe(
        "second",
      );
      expect(structured.primaryError.message).toContain("remote primary");
      expect(structured.rollbackErrors.map((error) => error.message)).toEqual([
        "remote rollback",
      ]);
    });
    /**
    ...
    it("should execute handler via BullMQ and return result", async () => {
      // Define the workflow
      const TestWorkflow = createWorkflowDefinition<
        { value: number },
        { doubled: number }
      >(
        "test-workflow",
      ...
      const handle = await emitterProvider.execute(TestWorkflow as any, {
        value: 21,
      });
    ...
    it("should propagate meta through distributed workflow", async () => {
      const MetaWorkflow = createWorkflowDefinition<
        { id: string },
        { processed: boolean }
      >(
        "meta-workflow",
    ...
    it("should handle workflow errors and propagate them", async () => {
      const ErrorWorkflow = createWorkflowDefinition<
        { shouldFail: boolean },
        { ok: boolean }
      >(
        "error-workflow",
      ...
      const handle = await emitterProvider.execute(ErrorWorkflow as any, {
        shouldFail: true,
      });

      // Result should reject with error
      await expect(
        withTimeout(handle.result(), TEST_TIMEOUTS.WORKFLOW_EXECUTION),
      ).rejects.toThrow(/Intentional workflow failure/);
    ...
    it("should support multiple concurrent workflows", async () => {
      const CounterWorkflow = createWorkflowDefinition<
        { count: number },
        { result: number }
      >(
        "counter-workflow",
      ...
      const results = await Promise.all(
        handles.map((h) =>
          withTimeout(h.result(), TEST_TIMEOUTS.WORKFLOW_EXECUTION),
        ),
      );
    ...
    it("should use idempotency key to prevent duplicate execution", async () => {
      const IdempotentWorkflow = createWorkflowDefinition<
        { value: number },
        { computed: number }
      >(
        "idempotent-workflow",
      ...
      const bothAdding = new Promise<void>(
        (resolve) => (releaseAdds = resolve),
      );
      class BarrierFlowProducer extends FlowProducer {
        public constructor(
          options: ConstructorParameters<typeof FlowProducer>[0],
        ) {
          super(options);
        }
      ...
      const [handle1, handle2] = await Promise.all([
        firstEmitter.execute(
          IdempotentWorkflow as any,
          { value: 100 },
          { idempotencyKey },
        ),
        secondEmitter.execute(
          IdempotentWorkflow as any,
          { value: 200 },
          { idempotencyKey },
        ),
  ...
  describe("Consumer-only mode", () => {
    it("should allow consumer to both emit and consume", async () => {
      const SelfWorkflow = createWorkflowDefinition<
        { input: string },
        { output: string }
      >(
        "self-workflow",
      ...
      const handle = await singleProvider.execute(SelfWorkflow as any, {
        input: "test",
      });
      const result = await withTimeout(
        handle.result(),
  ...
  describe("Step Execution (CRITICAL - Full IWorkflowConsumer Pattern)", () => {
    ...
    it("should execute steps configured via IWorkflowConsumer.configure() through BullMQ", async () => {
      ...
      const StepWorkflow = createWorkflowDefinition<
        { value: number },
        { finalResult: number }
      >(
        "step-workflow",
      ...
      const handle = await singleProvider.execute(StepWorkflow as any, {
        value: 5,
      });
      const result = await withTimeout(
        handle.result(),
      ...
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "double" }),
      );
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "addTen" }),
      );
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "onComplete" }),
      );
    });
    ...
    it("should execute step rollbacks on failure through BullMQ", async () => {
      ...
      const RollbackWorkflow = createWorkflowDefinition<
        { shouldFail: boolean },
        void
      >(
        "rollback-workflow",
      ...
      const handle = await singleProvider.execute(RollbackWorkflow as any, {
        shouldFail: true,
      });

      await expect(
        withTimeout(handle.result(), TEST_TIMEOUTS.WORKFLOW_EXECUTION),
      ).rejects.toThrow("Step2 intentionally failed");
      ...
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "step2" }),
      );
      ...
      expect(stepExecutionLog).not.toContainEqual(
        expect.objectContaining({ step: "onComplete" }),
      );
    });
    ...
    it("should execute parallel steps concurrently through BullMQ", async () => {
      ...
      const ParallelWorkflow = createWorkflowDefinition<
        { value: number },
        { sum: number }
      >(
        "parallel-workflow",
      ...
      const handle = await singleProvider.execute(ParallelWorkflow as any, {
        value: 10,
      });
      const result = await withTimeout(
        handle.result(),
    ...
    it("should pass step results to onComplete in distributed emitter/consumer setup", async () => {
      ...
      const stepHandlers = {
        ...
        process: {
          execute: async (ctx: WorkflowContext<{ userId: string }>) => {
            const validateResult = ctx.results["validate"] as {
              validated: boolean;
              timestamp: number;
            };
            stepExecutionLog.push({
      ...
      consumerProvider.registerDefinitionConsumer(
        ...
        async (_data, _meta, stepResults) => {
          ...
          const validateResult = stepResults?.["validate"] as {
            validated: boolean;
            timestamp: number;
          };
          if (!validateResult) {
            throw new Error(
              "validateResult is undefined - step results not passed to onComplete",
            );
          }
      ...
      const handle = await emitterProvider.execute(
        DistributedStepWorkflow as any,
        { userId: "test-user" },
      );
      const result = await withTimeout(
        handle.result(),
      ...
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "validate" }),
      );
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "process" }),
      );
      expect(stepExecutionLog).toContainEqual(
        expect.objectContaining({ step: "onComplete" }),
      );
      ...
      expect((result as { processedAt: number }).processedAt).toBeGreaterThan(
        0,
      );
    });

    it("should distribute step execution across multiple consumer instances", async () => {
      const DistributedWorkflow = createWorkflowDefinition<
        { items: number[] },
        { processed: number }
      >(
      ...
      consumerProvider.registerDefinitionConsumer(
        "distributed-workflow",
        registerHandler("consumer1"),
      );
      consumer2Provider.registerDefinitionConsumer(
        "distributed-workflow",
        registerHandler("consumer2"),
      );
      ...
      const handles = await Promise.all(
        Array.from({ length: 12 }, (_, index) =>
          emitterProvider.execute(DistributedWorkflow as any, {
            items: [index],
          }),
        ),
      );
      ...
      const consumer1Work = stepExecutionLog.filter(
        (e) => e.instance === "consumer1",
      );
      const consumer2Work = stepExecutionLog.filter(
        (e) => e.instance === "consumer2",
      );
