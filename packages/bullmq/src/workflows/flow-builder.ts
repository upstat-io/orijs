...
export class FlowBuilder {
  ...
  private buildChildrenChain<TData>(
    ...
    for (const group of stepGroups) {
      childrenForNextGroup = this.processGroup(
        group,
        workflowData,
        childrenForNextGroup,
      );
    }
  ...
  private createStepJob<TData>(
    ...
    const stepJobId = this.idempotencyKey
      ? `${this.idempotencyKey}-step-${stepDef.name}`
      : undefined;
