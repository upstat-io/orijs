...
function validateStepName(stepName: string, workflowName: string): void {
  if (!stepName || typeof stepName !== "string") {
    throw new Error(
      `Invalid step name in workflow '${workflowName}': step name must be a non-empty string`,
    );
  }
...
export interface BullMQWorkflowProviderOptions {
  ...
  readonly FlowProducerClass?: new (opts: {
...
export class BullMQWorkflowProvider implements WorkflowProvider<never> {
  ...
  private readonly workflowOptions: Readonly<
    Record<string, BullMQWorkflowOptions>
  >;
  private readonly log: Logger;

  // Minimal local state - only for caller-side tracking
  private readonly localFlowStates: Map<string, LocalFlowState> = new Map();
  private readonly timeoutHandles: Map<string, ReturnType<typeof setTimeout>> =
    new Map();
  // Handles for scheduled localFlowStates cleanup (cleared on stop)
  private readonly flowStateCleanupHandles: Map<
    string,
  ...
  private readonly FlowProducerClass: new (opts: {
    connection: ConnectionOptions;
  }) => IFlowProducer;
  ...
  public constructor(options: BullMQWorkflowProviderOptions) {
    ...
    this.flowStateCleanupDelay =
      options.flowStateCleanupDelay ?? DEFAULT_FLOW_STATE_CLEANUP_DELAY_MS;
    ...
    this.FlowProducerClass =
      options.FlowProducerClass ??
      (FlowProducer as unknown as typeof this.FlowProducerClass);
    this.WorkerClass =
      options.WorkerClass ?? (Worker as unknown as typeof this.WorkerClass);
    this.QueueEventsClass =
      options.QueueEventsClass ??
      (QueueEvents as unknown as typeof this.QueueEventsClass);
    this.QueueClass =
  ...
  public async execute<TData, TResult>(
    ...
    const isRegistered =
      this.definitionConsumers.has(workflowName) ||
      this.emitterWorkflows.has(workflowName);
    if (!isRegistered) {
  ...
  private async executeDefinition<TData, TResult>(
    ...
    const handleKey = idempotencyKey
      ? `${workflowName}:${idempotencyKey}`
      : undefined;
    const existingHandle = handleKey
      ? this.idempotentHandles.get(handleKey)
      : undefined;
    ...
    if (idempotencyKey) {
      ...
      if (existing.job) {
        const existingHandle = this.createDurableHandle<TResult>(
          existing.job,
          queueName,
          idempotencyKey,
        );
    ...
    const {
      ...
      resolve,
      reject,
    } = this.createDeferredResult<TResult>();
    ...
    if (handleKey)
      this.idempotentHandles.set(handleKey, handle as FlowHandle<unknown>);
    ...
    const stepJobOpts: StepJobRetryOpts = {
      ...
      ...(workflowPolicy?.priority !== undefined && {
    ...
    if (idempotencyKey) {
      ...
      const durableFlowId = (durable.job?.data as WorkflowJobData | undefined)
        ?.flowId;
      if (durable.job && durableFlowId && durableFlowId !== flowId) {
        this.pendingResults.delete(jobId);
        this.localFlowStates.delete(flowId);
        const durableHandle = this.createDurableHandle<TResult>(
          durable.job,
          queueName,
          idempotencyKey,
        );
        ...
        if (handleKey)
          this.idempotentHandles.set(
            handleKey,
            durableHandle as FlowHandle<unknown>,
          );
  ...
  private createDurableHandle<TResult>(
    job: Job,
    queueName: string,
    jobId: string,
  ): FlowHandle<TResult> {
  ...
  public async getResult<TResult = unknown>(flowId: string): Promise<TResult> {
    ...
    if (!found) {
      throw new Error(
        `Workflow '${flowId}' not found. It may not exist or has been cleaned up.`,
      );
    }
   ...
   */
  public async getHandle<TResult = unknown>(
    flowId: string,
  ): Promise<FlowHandle<TResult>> {
   ...
   */
  private async findJobByFlowId(
    flowId: string,
  ): Promise<{ job: Job; queue: Queue } | null> {
    ...
    const workflowNames = new Set<string>([
      ...this.definitionConsumers.keys(),
      ...this.emitterWorkflows,
    ]);
  ...
  private async storeWorkflowFailure(
    flowId: string,
    error: Error,
  ): Promise<void> {
    ...
    await client
      .set(
        this.getFlowFailureKey(flowId),
        error.message,
        "EX",
        FLOW_REGISTRY_TTL_SECONDS,
      )
      .catch(() => {});
  }

  private async loadWorkflowFailure(
    flowId: string,
    fallback: string,
  ): Promise<Error> {
    const client = this.getRedisRegistryClient();
  ...
  private getRedisRegistryClient(): IRedisRegistryClient | null {
    return (
      (this.flowProducer?.connection?._client as IRedisRegistryClient) ?? null
    );
  }
   ...
   */
  private async registerFlowInRegistry(
    flowId: string,
    workflowName: string,
  ): Promise<void> {
    const client = this.getRedisRegistryClient();
    ...
    try {
      await client.set(
        this.getFlowRegistryKey(flowId),
        workflowName,
        "EX",
        FLOW_REGISTRY_TTL_SECONDS,
      );
   ...
   */
  private async waitForJobCompletion<TResult>(
    job: Job,
    flowId: string,
    expectedJobId = flowId,
  ): Promise<TResult> {
  ...
  public registerDefinitionConsumer(
    ...
    stepHandlers?: Record<
      string,
      { execute: (ctx: any) => any; rollback?: (ctx: any) => any }
    >,
    onError?: (
      data: unknown,
      meta?: PropagationMeta,
    ...
    if (groups.length > 0) {
      if (stepHandlers && Object.keys(stepHandlers).length > 0) {
        for (const group of groups) {
          for (const stepDef of group.definitions) {
            ...
            this.stepRegistry.register(
              workflowName,
              stepName,
              handlerEntry.execute as (
                ctx: WorkflowContext<unknown>,
              ) => Promise<unknown>,
              handlerEntry.rollback as
                | ((ctx: WorkflowContext<unknown>) => Promise<void> | void)
                | undefined,
            );
          }
        }
        ...
        this.log.warn(
          `Workflow '${workflowName}' has steps defined but no stepHandlers provided`,
        );
        this.log.info(
  ...
  public async start(): Promise<void> {
    ...
    if (consumerList.length > 0) {
      ...
      const emitterDisplay = emitterList
        .map((name) => `${name} (Non Consumer)`)
        .join(", ");
  ...
  public async stop(): Promise<void> {
    ...
    if (this.flowProducer) {
      if (this.flowProducer.connection?._client) {
        this.flowProducer.connection._client.on(
          "error",
          errorHandler as (...args: unknown[]) => void,
        );
      }
  ...
  private async pauseWorkers(): Promise<void> {
    const workers = [
      ...this.stepWorkers.values(),
      ...this.workflowWorkers.values(),
    ];
  ...
  private async closeWorkerMap(
    ...
    const closes = Array.from(workers.values(), (worker) => {
      worker.connection._client.on(
        "error",
        errorHandler as (...args: unknown[]) => void,
      );
      worker.blockingConnection._client.on(
        "error",
        errorHandler as (...args: unknown[]) => void,
      );
  ...
  private async closeQueueEventsInitializations(): Promise<void> {
    const initializations = Array.from(
      this.queueEventsInitializations.values(),
    );
    try {
      await Promise.all(
        initializations.map((initialization) => initialization.close()),
      );
    } finally {
      await Promise.allSettled(
        initializations.map((initialization) => initialization.ready),
      );
  ...
  private createDefinitionWorker(workflowName: string): void {
    ...
    const workerOpts = this.buildWorkerOptions(
      this.workflowOptions[workflowName],
    );
  ...
  private buildWorkerOptions(options?: BullMQWorkflowOptions): {
    ...
    const effectiveStallInterval =
      this.stallInterval ?? DEFAULT_STALL_INTERVAL_MS;
    return {
      connection: this.connection,
  ...
  private createWorkflowWorkerIfNeeded(
    ...
    const worker = new this.WorkerClass(
      queueName,
      this.trackWorkerProcessor(processor),
      workerOpts,
    );
    worker.on("error", (err: unknown) => {
  ...
  private initializeFlowState(
    ...
    if (!meta) {
      this.log.debug(
        "Workflow executing without propagation metadata (no correlation ID)",
        { flowId },
      );
    }
   ...
   */
  private async processDefinitionWorkflow(
    job: Job,
    workflowName: string,
  ): Promise<unknown> {
    ...
    if (localState?.status === "failed") {
      this.log.debug(
        "Definition workflow already failed locally, skipping onComplete",
        { flowId },
      );
      return undefined;
    }
    ...
    try {
      ...
      if (consumer.stepGroups.length > 0) {
        ...
        if (Object.keys(stepResults).length === 0) {
          const stepNames = consumer.stepGroups.flatMap((g) =>
            g.definitions.map((d) => d.name),
          );
          throw new Error(
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new WorkflowStepError(
            stepName,
            new Error(
              `Step '${stepName}' timed out after ${this.stepTimeout}ms`,
            ),
          ),
        );
      }, this.stepTimeout);
  ...
  private async processStep(
    job: Job,
  ): Promise<StepResultWrapper | ParallelResultWrapper> {
    ...
    try {
      ...
      const failure =
        rollbackErrors.length > 0
          ? new WorkflowRollbackError(stepError, rollbackErrors)
          : stepError;
  ...
  private async runRollbacks(ctx: RollbackContext): Promise<Error[]> {
    ...
    for (const stepName of completedStepNames.reverse()) {
      const rollback = this.stepRegistry.getRollback(
        ctx.workflowName,
        stepName,
      );
  ...
  private createContext(
    ...
    const baseLog = meta
      ? Logger.fromMeta(workflowName, meta)
      : this.log.child(workflowName);
  ...
  private async getOrCreateQueueEvents(
    ...
    const ready = (async (): Promise<IQueueEvents> => {
      ...
      try {
        ...
        if (!this.started) {
          throw new Error(
            "Workflow provider stopped while QueueEvents was initializing",
          );
        }
        ...
        events.on("failed", async (...eventArgs: unknown[]) => {
          ...
          // Update local state
          const localState = this.localFlowStates.get(pending.flowId);
          const failure = await this.loadWorkflowFailure(
            pending.flowId,
            failedReason,
          );
          if (localState) {
   ...
   */
  private handleTimeoutWithRedisCheck(
    flowId: string,
   ...
   */
  private async checkRedisAndTimeout(
    flowId: string,
    effectiveTimeout: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (redisState === "failed") {
        ...
        this.log.debug(
          "Timeout avoided - job failed in Redis (QueueEvents will handle)",
          { flowId },
        );
...
export function createBullMQWorkflowProvider(
  options: BullMQWorkflowProviderOptions,
