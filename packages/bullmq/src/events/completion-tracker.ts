...
export class CompletionTracker implements ICompletionTracker {
  ...
  private readonly jobIdToCorrelationId = new Map<
    string,
  ...
  public getCorrelationId(
    ...
    return this.jobIdToCorrelationId.get(queueName)?.get(jobId)?.values().next()
      .value;
  }
  ...
  public completeJob(queueName: string, jobId: string, result: unknown): void {
    const correlationIds = [
      ...(this.jobIdToCorrelationId.get(queueName)?.get(jobId) ?? []),
    ];
