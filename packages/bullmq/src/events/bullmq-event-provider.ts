...
export class BullMQEventProvider implements EventProvider {
  ...
  public constructor(options: BullMQEventProviderOptions) {
    ...
    this.completionTracker =
      options.completionTracker ??
      new CompletionTracker({ connection: this.connection });
  ...
  public emit<TReturn = void>(
    ...
    const jobOptions = {
      ...
      ...(ttl !== undefined && {
        removeOnFail: { age: Math.ceil(ttl / 1000) },
      }),
    };
    ...
    if (options?.expectsResult === false) {
      const enqueue = async () => {
        if (
          ...
          (await this.queueManager.hasCompletionReceipt?.(
            eventName,
            options.idempotencyKey,
          ))
        ) {
    ...
    const enqueue = async () => {
      ...
      this.completionTracker.mapJobId(
        queueName,
        jobOptions.jobId,
        subscription.correlationId,
      );
      ...
      if (state === "completed") {
        const retainedJob =
          (await this.queueManager.getJob?.(eventName, job.id)) ?? job;
        this.completionTracker.completeJob(
          queueName,
          job.id,
          retainedJob.returnvalue,
        );
  ...
  public async subscribe<TPayload = unknown, TReturn = void>(
    ...
    const workerHandler = async (job: {
      ...
      if (job.data.idempotencyKey) {
        await this.queueManager.recordCompletionReceipt?.(
          eventName,
    ...
    await this.queueManager.registerWorker(
      eventName,
      workerHandler,
      (jobId, result) => {
        this.completionTracker.completeJob(
  ...
  public async prepareIdempotencyKeyRetirement(
    eventName: string,
    key: string,
  ): Promise<void> {
    await this.queueManager.prepareCompletionReceiptRetirement?.(
      eventName,
      key,
    );
  }

  public async finalizeIdempotencyKeyRetirement(
    eventName: string,
    key: string,
  ): Promise<void> {
    await this.queueManager.finalizeCompletionReceiptRetirement?.(
      eventName,
      key,
    );
  }

  public async hasSuccessfulIdempotencyKeyCompletionReceipt(
    eventName: string,
    key: string,
  ): Promise<boolean> {
    return (
      (await this.queueManager.hasSuccessfulCompletionReceipt?.(
        eventName,
        key,
      )) ?? false
    );
  ...
  public async isRetainedEventRetryable(
    eventName: string,
    eventId: string,
  ): Promise<boolean> {
    return (
      (await this.queueManager.isJobRetryable?.(eventName, eventId)) ?? true
    );
  }
  ...
  private async waitForCompletionOwnership(queueName: string): Promise<void> {
    ...
    const onStop = () =>
      rejectStop(new Error("BullMQEventProvider is stopping"));
    signal.addEventListener("abort", onStop, { once: true });
    try {
      await Promise.race([
        this.completionTracker.waitUntilReady(queueName),
        stopped,
      ]);
    } finally {
  ...
  private sweepStaleJobs(): void {
    for (const [eventName, ttl] of this.eventTtls) {
      this.queueManager
        .cleanJobs(eventName, ttl, this.sweepBatchLimit, "wait")
        .catch(() => {});
      this.queueManager
        .cleanJobs(eventName, ttl, this.sweepBatchLimit, "failed")
        .catch(() => {});
    }
  }
