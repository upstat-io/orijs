...
export class SocketCoordinator {
  ...
  private readonly connections: Map<string, WebSocketConnection<unknown>> =
    new Map();
  ...
  public subscribeToTopic(socketId: string, topic: string): void {
    ...
    if (!ws) {
      this.logger.warn?.(
        `Cannot subscribe socket ${socketId} to topic "${topic}": socket not found`,
      );
      return;
    }
   ...
   */
  public getConnection<TData>(
    socketId: string,
  ): WebSocketConnection<TData> | undefined {
    return this.connections.get(socketId) as
      WebSocketConnection<TData> | undefined;
  }
