...
describe("InProcWsProvider", () => {
  ...
  describe("publish to topics", () => {
    it("should log error and reject when server not set", async () => {
      ...
      await expect(loggerProvider.publish("topic", "message")).rejects.toThrow(
        "Provider not ready",
      );
  ...
  describe("send", () => {
    ...
    it("should publish to socket-specific channel and return true", () => {
      ...
      expect(publishMock).toHaveBeenCalledWith(
        `__socket__:${validSocketId}`,
        "direct-message",
      );
    });

    it("should throw on invalid socket ID format", () => {
      expect(() => provider.send("socket-123", "message")).toThrow(
        "Invalid socket ID format",
      );
      expect(() => provider.send("not-a-uuid", "message")).toThrow(
        "Invalid socket ID format",
      );
    });

    it("should throw on empty socket ID", () => {
      expect(() => provider.send("", "message")).toThrow(
  ...
  describe("subscribe", () => {
    ...
    it("should throw on invalid socket ID format", () => {
      expect(() => provider.subscribe("invalid-socket", "topic-1")).toThrow(
        "Invalid socket ID format",
      );
    });

    it("should throw on empty socket ID", () => {
      expect(() => provider.subscribe("", "topic-1")).toThrow(
        "Socket ID cannot be empty",
      );
    });
  });
  ...
  describe("disconnect", () => {
    ...
    it("should throw on invalid socket ID format", () => {
      expect(() => provider.disconnect("invalid-socket")).toThrow(
        "Invalid socket ID format",
      );
    });

    it("should throw on empty socket ID", () => {
      expect(() => provider.disconnect("")).toThrow(
        "Socket ID cannot be empty",
      );
    });
  });
  ...
  describe("unsubscribe", () => {
    ...
    it("should throw on invalid socket ID format", () => {
      expect(() => provider.unsubscribe("invalid-socket", "topic")).toThrow(
        "Invalid socket ID format",
      );
    });
    ...
    it("should handle unsubscribe for non-existent topic", () => {
      provider.subscribe(SOCKET_1, "topic-1");
      expect(() =>
        provider.unsubscribe(SOCKET_1, "unknown-topic"),
      ).not.toThrow();
    });
  ...
  describe("automatic disconnection on unsubscribe", () => {
    ...
    it("should throw on invalid socket ID", () => {
      expect(() => provider.unsubscribe("invalid-socket", "topic-1")).toThrow(
        "Invalid socket ID format",
      );
    });
  });
  ...
  describe("stop cleanup", () => {
    ...
    it("should clear server reference on stop", async () => {
      ...
      await expect(testProvider.publish("topic", "after-stop")).rejects.toThrow(
        "Provider not ready",
      );
      expect(errorMock).toHaveBeenCalled();
  ...
  describe("topic validation", () => {
    it("should throw on empty topic for subscribe", () => {
      expect(() => provider.subscribe(SOCKET_1, "")).toThrow(
        "Topic cannot be empty",
      );
    });

    it("should throw on empty topic for unsubscribe", () => {
      expect(() => provider.unsubscribe(SOCKET_1, "")).toThrow(
        "Topic cannot be empty",
      );
    });

    it("should throw on empty topic for publish", () => {
      expect(() => provider.publish("", "message")).toThrow(
        "Topic cannot be empty",
      );
    });
    ...
    it("should throw on topic exceeding max length", () => {
      ...
      expect(() => provider.subscribe(SOCKET_1, longTopic)).toThrow(
        "Topic name too long",
      );
    });
    ...
    it("should accept valid topic names", () => {
      expect(() =>
        provider.subscribe(SOCKET_1, "valid:topic:name"),
      ).not.toThrow();
      expect(() =>
        provider.subscribe(SOCKET_1, "topic-with-dashes"),
      ).not.toThrow();
      expect(() =>
        provider.subscribe(SOCKET_1, "topic_with_underscores"),
      ).not.toThrow();
    });
  });
});
