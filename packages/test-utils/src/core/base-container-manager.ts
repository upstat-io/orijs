...
export abstract class BaseContainerManager {
  ...
  async start(): Promise<StartedTestContainer> {
    ...
    if (this.isCircuitBreakerOpen()) {
      throw new Error(
        `Circuit breaker is open for ${this.packageName} container`,
      );
    }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `Container stop timed out for ${this.packageName}, forcing cleanup:`,
        error,
      );
      try {
  ...
  private async executeWithRetry<T>(operation: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        this.onFailure();
        console.warn(
          `Container start attempt ${attempt} failed for ${this.packageName}:`,
          lastError.message,
        );
  ...
  private async verifyContainerHealth(): Promise<void> {
    ...
    for (let i = 0; i < maxChecks; i++) {
      try {
        if (await this.withTimeout(this.performHealthCheck(), 3000)) {
          ...
          console.log(
            `Container health verified for ${this.packageName} after ${i + 1} checks`,
          );
          return;
        }
      } catch (error) {
        console.warn(
          `Health check ${i + 1} failed for ${this.packageName}:`,
    ...
    throw new Error(
      `Container failed health verification after ${maxChecks} attempts: ${this.packageName}`,
    );
  }
  ...
  private async cleanupFailedContainers(): Promise<void> {
    try {
      ...
      console.warn(
        `Failed to cleanup failed containers for ${this.packageName}:`,
        error,
      );
    }
  }
  ...
  private async aggressiveCleanup(): Promise<void> {
    try {
      ...
      execSync(
        `docker ps -a --filter "env=POSTGRES_DB=${dbName}" -q | xargs -r docker kill`,
        {
          stdio: "ignore",
        },
      );
      execSync(
        `docker ps -a --filter "env=POSTGRES_DB=${dbName}" -q | xargs -r docker rm -f`,
        {
          stdio: "ignore",
        },
      );
  ...
  private withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(
          new Error(
            `Operation timed out after ${timeoutMs}ms for ${this.packageName} container`,
          ),
        );
      }, timeoutMs);
    });
