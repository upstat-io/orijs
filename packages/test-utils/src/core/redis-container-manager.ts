...
export class RedisContainerManager extends BaseContainerManager {
  ...
  protected async createContainer(): Promise<StartedRedisContainer> {
    ...
    const enableReuse =
      !process.env.CI && process.env.TESTCONTAINERS_REUSE !== "false";
    ...
    console.log(
      `Redis container ready for ${this.packageName} on port ${this.redisContainer.getPort()}`,
    );
  ...
  setupNestJSEnvironment(): { host: string; port: number; db: number } {
    ...
    if (config.db === undefined)
      throw new Error("Redis test database lease is unavailable");
  ...
  getRedisClient(): Redis {
    if (!this.redisClient) {
      throw new Error(
        `Redis client not available for ${this.packageName} - container not started`,
      );
    }
  ...
  async cleanup(): Promise<void> {
    try {
      // Clean up queues
      const cleanupPromises = Array.from(this.queues.values()).map(
        async (queue) => {
          try {
            ...
            console.debug(
              `Queue cleanup warning for ${this.packageName}:`,
              error,
            );
          }
        },
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("Redis cleanup timeout")), 3000),
        ),
      ]);
