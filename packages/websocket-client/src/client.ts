/**
 * Browser WebSocket client for OriJS.
 *
 * Provides a type-safe interface for consuming WebSocket messages from an OriJS server.
 ...
 *
 * // Define your message types (or import from your message definitions package)
...
export class SocketClient {
  ...
  connect(): void {
    ...
    socket.onclose = (event) => {
      ...
      if (event.code === 1008 || event.code === 1011) {
        this.notifyError(
          new Error(
            `Connection rejected (${event.code}: ${event.reason || "unknown"})`,
          ),
        );
        return;
      }
  ...
  destroy(): void {
    ...
    if (
      win?.document &&
      typeof win.document.removeEventListener === "function"
    ) {
   ...
   */
  on<TData>(
    message: ClientMessageDefinition<TData>,
   ...
   * @example
   * ```typescript
   * import { JoinRoom } from '@orijs/websocket-client';
   *
  ...
  private handleMessage(data: string): void {
    ...
    try {
      ...
      // Matches the pattern used in RequestContext.json()
