...
import { SocketClient, Connected, Disconnected } from "../src/client.ts";
import type {
...
describe("SocketClient with Real WebSocket", () => {
  ...
  describe("room management", () => {
    it("should send joinRoom message to server when connected", async () => {
      ...
      await waitFor(() => getServerMessages("room.join").length > 0, {
        timeout: 1000,
      });

      const joinMessages = getServerMessages("room.join");
      expect(joinMessages.length).toBe(1);
      expect((joinMessages[0]!.parsed as { room: string }).room).toBe(
        "account:123",
      );
    });
  ...
  describe("room auto-rejoin on reconnect", () => {
    it("should auto-rejoin rooms after server-initiated disconnect", async () => {
      ...
      await waitFor(() => getServerMessages("room.join").length === 2, {
        timeout: 1000,
      });
      ...
      const rooms = joinMessages
        .map((m) => (m.parsed as { room: string }).room)
        .toSorted();
    ...
    it("should auto-rejoin rooms on every reconnect cycle", async () => {
      ...
      await waitFor(() => getServerMessages("room.join").length === 1, {
        timeout: 1000,
      });
  ...
  describe("heartbeat", () => {
    it("should send heartbeat and receive pong", async () => {
      ...
      // Wait for heartbeat
      await waitFor(() => receivedMessages.some((m) => m.data === PING_FRAME), {
        timeout: 500,
      });
  ...
  describe("message handling", () => {
    ...
    it("should sanitize prototype pollution keys from incoming messages", async () => {
      ...
      expect(Object.prototype.hasOwnProperty.call(data, "__proto__")).toBe(
        false,
      );
      expect(Object.prototype.hasOwnProperty.call(data, "constructor")).toBe(
        false,
      );
      expect(Object.prototype.hasOwnProperty.call(data, "prototype")).toBe(
        false,
      );
  ...
  describe("debug: exact reconnect flow", () => {
    it("should log exact sequence of events during reconnect", async () => {
      ...
      events.push(
        `initial joinRoom sent, rooms tracked: ${[...client.joinedRooms].join(",")}`,
      );
      ...
      await waitFor(() => client.isConnected, { timeout: 2000 });
      events.push(
        `reconnected, rooms tracked: ${[...client.joinedRooms].join(",")}`,
      );
