 ...
 */
export async function readLogConfig(
  ...
  return {
    ...
    fileMaxFiles: fileMaxCount
      ? parseInt(fileMaxCount, 10)
...
export function buildLoggerOptions(config: LogConfig): LoggerOptions {
  ...
  return {
    level: config.level,
    transports:
      transports.length > 1 ? [multiTransport(transports)] : transports,
  };
}
 ...
 * ```
 */
export async function createLoggerOptionsFromConfig(
  config: ConfigProvider,
