...
describe("requestContext", () => {
  ...
  test("should return context when set via runWithContext", async () => {
    ...
    await runWithContext(
      { log: testLogger, correlationId: testRequestId },
      async () => {
        const ctx = requestContext();
...
describe("capturePropagationMeta", () => {
  ...
  test("should create child trace span when trace context exists", async () => {
    ...
    await runWithContext(
      { log: testLogger, correlationId: testRequestId, trace },
      async () => {
        const meta = capturePropagationMeta();
  ...
  test("should create unique child spans for each capture", async () => {
    ...
    await runWithContext(
      { log: testLogger, correlationId: "req-1", trace },
      async () => {
  ...
  test("should capture application-injected metadata from setMeta", async () => {
    ...
    await runWithContext(
      { log: testLogger, correlationId: "req-meta-test", trace },
      async () => {
    ...
    await runWithContext(
      {
        ...
        meta: {
          userId: hop1Received.userId,
          accountUuid: hop1Received.accountUuid,
        },
      },
      async () => {
      ...
      await runWithContext(
        {
          ...
          meta: {
            userId: receivedA.userId,
            accountUuid: receivedA.accountUuid,
          },
        },
        async () => {
          const ctx = requestContext();
      ...
      await runWithContext(
        {
          ...
          meta: {
            userId: receivedB.userId,
            accountUuid: receivedB.accountUuid,
          },
        },
        async () => {
          const ctx = requestContext();
  ...
  test("should propagate context set via Logger.setMeta across services", async () => {
    ...
    await runWithContext(
      ...
      async () => {
        ...
        expect(ctx.log.propagationMeta().accountUuid).toBe(
          "account-from-logger",
        );
      },
    );
