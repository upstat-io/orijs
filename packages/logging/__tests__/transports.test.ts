...
describe("consoleTransport", () => {
  ...
  const inlineCases: Array<{
    name: string;
    fields: Partial<LogObject>;
    expect: string[];
  }> = [
    {
      name: "string under error",
      fields: { error: "str-a" },
      expect: ["error:str-a"],
    },
    {
      name: "string under err",
      fields: { err: "str-b" },
      expect: ["err:str-b"],
    },
    {
      name: "string under both",
      fields: { error: "str-a", err: "str-b" },
      expect: ["error:str-a", "err:str-b"],
    },
    { name: "number under error", fields: { error: 42 }, expect: ["error:42"] },
    { name: "number under err", fields: { err: 7 }, expect: ["err:7"] },
    {
      name: "boolean under error",
      fields: { error: false },
      expect: ["error:false"],
    },
    {
      name: "array under error",
      fields: { error: [1, 2, 3] },
      expect: ["error:[1, 2, 3]"],
    },
    {
      name: "array under err",
      fields: { err: ["x", "y"] },
      expect: ["err:[x, y]"],
    },
    {
      name: "object under error",
      fields: { error: { code: "E1" } },
      expect: ["error:", "code"],
    },
    {
      name: "object under err",
      fields: { err: { code: "E2" } },
      expect: ["err:", "code"],
    },
  ];
  ...
  test("should render an Error under error as a trailing block, not inline", () => {
    const output = captureWrite(
      createLogObject({ error: new Error("err-instance-a") }),
    );
  ...
  test("should render an Error under err as a trailing block, not inline", () => {
    const output = captureWrite(
      createLogObject({ err: new Error("err-instance-b") }),
    );
    const [head, ...rest] = output.split("\n");
  ...
  test("should render a string inline and an Error as a block when both keys are populated", () => {
    const output = captureWrite(
      createLogObject({ error: "inline-str", err: new Error("block-err") }),
    );
    const [head, ...rest] = output.split("\n");
  ...
  test("should label both blocks by key when error and err both hold Errors", () => {
    const output = captureWrite(
      createLogObject({
        error: new Error("two-block-a"),
        err: new Error("two-block-b"),
      }),
    );
  ...
  test("should not wrap the trailing error block in red below error level", () => {
    const output = captureWrite(
      createLogObject({
        level: levels.info,
        error: new Error("not-red-wrapped"),
      }),
      { colors: true },
    );
    const block = output.slice(output.indexOf("\n") + 1);
  ...
  test("should leave json mode output unchanged for every reserved-key shape", () => {
    ...
    try {
      for (const cell of inlineCases)
        transport.write(createLogObject({ time: 0, ...cell.fields }));
    } finally {
      console.log = original;
    }
    for (const [index, line] of lines.entries()) {
      expect(line).toBe(
        JSON.stringify(
          createLogObject({ time: 0, ...inlineCases[index]!.fields }),
        ),
      );
    }
  });
...
describe("fileTransport", () => {
  ...
  describe("size parsing", () => {
    test("should parse kb size correctly", async () => {
      ...
      for (let i = 0; i < 20; i++) {
        transport.write(
          createLogObject({
            msg: `Message number ${i} with some extra padding to make it longer`,
          }),
        );
      }
  ...
  describe("file rotation", () => {
    ...
    test("should shift existing rotated files correctly", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        transport.write(
          createLogObject({ msg: `Filler ${i} to trigger rotation` }),
        );
      }
...
describe("multiTransport", () => {
  test("should write to all transports", () => {
    ...
    const transport = multiTransport([
      {
        write: (obj) => logs1.push(obj),
        flush: async () => {},
        close: async () => {},
      },
      {
        write: (obj) => logs2.push(obj),
        flush: async () => {},
        close: async () => {},
      },
    ]);
