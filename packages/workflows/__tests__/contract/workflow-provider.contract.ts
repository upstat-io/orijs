  ...
  provider.registerDefinitionConsumer!(
    ...
    config.stepHandlers as Record<
      string,
      {
        ...
        rollback?: (ctx: unknown) => Promise<void>;
      }
    >,
    config.onError as
      | ((
 ...
 * ```
 */
export function workflowProviderContractTests(
  ...
  describe(`WorkflowProvider Contract: ${providerName}`, () => {
      ...
      it(
        ...
        async () => {
          ...
          const workflowConfig =
            createParallelAccumulatingWorkflow(capturedResults);
          const definition = registerAndGetDefinition(provider, workflowConfig);
          await provider.start();
    ...
    describe("mixed sequential and parallel", () => {
      it(
        ...
        async () => {
          ...
          expect(executionLog.indexOf("finalize")).toBeGreaterThan(
            executionLog.indexOf("task-a"),
          );
          expect(executionLog.indexOf("finalize")).toBeGreaterThan(
            executionLog.indexOf("task-b"),
          );

          // onComplete is last
          expect(executionLog.indexOf("onComplete")).toBe(
    ...
    describe("error handling", () => {
      it(
        ...
        async () => {
          ...
          const workflowConfig = createErrorCallbackWorkflow(
            executionLog,
            capturedError,
          );
          const definition = registerAndGetDefinition(provider, workflowConfig);
      ...
      it(
        ...
        async () => {
          ...
          expect(capturedResults.value["step1"]).toEqual({
            step1Result: "first",
          });
          expect(capturedResults.value["step2"]).toEqual({
            step2Result: "second",
          });
    ...
    describe("rollback on step failure", () => {
      ...
      it(
        ...
        async () => {
          ...
          expect(
            (failure as WorkflowRollbackError).primaryError,
          ).toBeInstanceOf(WorkflowStepError);
          expect(
            (
              (failure as WorkflowRollbackError)
                .primaryError as WorkflowStepError
            ).cause,
          ).toBe(primary);
      ...
      it(
        ...
        async () => {
          ...
          const workflowConfig = createContextRollbackWorkflow(
            executionLog,
            capturedContext,
          );
          const definition = registerAndGetDefinition(provider, workflowConfig);
          ...
          expect(capturedContext.results["step1"]).toEqual({
            step1Result: "value1",
          });
        },
        timeout,
    ...
    describe("workflow execution timeout", () => {
      it(
        ...
        async () => {
          const timeoutProvider = await createProviderWithConfig({
            timeoutMs: 5000,
          });
          const workflowConfig = createFastWorkflow();
          const definition = registerAndGetDefinition(
            timeoutProvider,
            workflowConfig,
          );
    ...
    describe("logger context propagation", () => {
      ...
      it(
        ...
        async () => {
          ...
          const loggerProvider = await createProviderWithConfig({
            logger: customLogger,
          });
