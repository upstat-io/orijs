...
const SlowWorkflow: WorkflowDefinitionLike<
  Record<string, never>,
  { done: boolean }
> = {
  name: "slow-workflow",
...
const QuickTimeoutWorkflow: WorkflowDefinitionLike<
  Record<string, never>,
  { success: boolean }
> = {
...
describe("Workflow timeout race", () => {
  test("should produce failed status when timeout fires during onComplete handler", async () => {
    ...
    provider.registerDefinitionConsumer(SlowWorkflow.name, async () => {
  ...
  test("should not resolve after timeout has already rejected", async () => {
    const provider = new InProcessWorkflowProvider();
    provider.registerDefinitionConsumer(QuickTimeoutWorkflow.name, async () => {
