...
export class InProcessWorkflowProvider implements WorkflowProvider {
  private readonly definitionConsumers: Map<string, DefinitionConsumerConfig> =
    new Map();
  private readonly flowStates: Map<string, FlowState<unknown>> = new Map();
  private readonly cleanupTimeouts: Map<string, ReturnType<typeof setTimeout>> =
    new Map();
    ...
    if (!this.definitionConsumers.has(workflowName)) {
      throw new Error(
        `Workflow '${workflowName}' not registered. ` +
          `Call registerDefinitionConsumer() first.`,
      );
    }
    return this.executeDefinitionWorkflow(
      workflowName,
      data,
      options?.timeout,
      options?.id,
  ...
  private async runRollbacks<TData>(
    ...
    for (const step of stepsToRollback) {
      const rollbackCtx = createWorkflowContext(
        ctx.flowId,
        ctx.data,
        ctx.results,
        ctx.log,
      ...
      try {
        ...
        rollbackCtx.log.info("Rollback Completed", {
          step: step.name,
          durationMs,
        });
      } catch (rollbackError) {
  ...
  private async executeDefinitionWorkflow<TData, TResult>(
    ...
    const existing = this.flowStates.get(flowId) as
      FlowState<TResult> | undefined;
    if (existing) {
      return {
        id: flowId,
    ...
    this.executeDefinitionWorkflowInternal(
      workflowName,
      data,
      flowId,
      flowState,
  ...
  private async executeDefinitionWorkflowInternal<TData, TResult>(
    ...
    const state = {
      ...
      completedStepsWithRollback: [] as Array<{
        name: string;
        rollback: RollbackHandler;
      }>,
    };
    ...
    const baseLog = capturedMeta
      ? Logger.fromMeta(workflowName, capturedMeta)
    ...
    for (const group of stepGroups) {
      ...
      const currentCtx: StepExecutionContext<TData> = {
        ...execCtx,
        results: state.results,
      };
      const stepError = await this.executeDefinitionStepGroup(
        group,
        currentCtx,
        state,
        stepHandlers,
      ...
      if (stepError) {
        ...
        workflowLog.error("Workflow Failed", {
          durationMs,
          step:
            stepError instanceof WorkflowStepError
              ? stepError.stepName
              : undefined,
  ...
  private async executeDefinitionStepGroup<TData>(
    ...
    if (group.type === "sequential") {
      return this.executeDefinitionSequentialGroup(
        group.definitions,
        ctx,
        state,
        stepHandlers,
      ...
      return this.executeDefinitionParallelGroup(
        group.definitions,
        ctx,
        state,
        stepHandlers,
  ...
  private async executeDefinitionSequentialGroup<TData>(
    ...
    for (const stepDef of definitions) {
      ...
      const stepCtx = createWorkflowContext(
        ctx.flowId,
        ctx.data,
        state.results,
        ctx.log,
      ...
      const stepError = await this.executeDefinitionStepSafely(
        stepDef.name,
        handler.execute,
        stepCtx,
        state,
      ...
      if (stepError) {
        ...
        const rollbackCtx: StepExecutionContext<TData> = {
          ...ctx,
          results: state.results,
        };
        const rollbackErrors = await this.runRollbacks(
          rollbackCtx,
          state.completedStepsWithRollback,
        );
  ...
  private async executeDefinitionParallelGroup<TData>(
    ...
    for (const item of parallelResults) {
      if ("error" in item) {
        if (!firstError) {
          firstError = new WorkflowStepError(
            item.name,
            (item as StepFailure).error,
          );
        }
      } else {
        ...
        state.results = {
          ...state.results,
          [item.name]: (item as StepSuccess).result,
        };
      }
    }
    ...
    if (firstError) {
      ...
      return rollbackErrors.length > 0
        ? new WorkflowRollbackError(firstError, rollbackErrors)
        : firstError;
    }
  ...
  private async executeDefinitionStepSafely<TData>(
    ...
    if (
      ...
      "__stepError" in resultOrError
    ) {
      const caught = resultOrError as CaughtError;
      ctx.log.debug("Step Failed", {
        durationMs,
