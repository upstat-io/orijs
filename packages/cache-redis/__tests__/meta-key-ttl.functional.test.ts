...
describe('RedisCacheProvider meta-key TTL', () => {
	...
	it('should not shorten a shared meta key TTL when a shorter-lived entry is written', async () => {
		...
		await redisCache.setWithMeta('cache:teams-collection', { teams: 19 }, LONG_TTL_SECONDS, [
			META_KEY
		]);
		await redisCache.setWithMeta('cache:team-detail', { team: 1 }, SHORT_TTL_SECONDS, [META_KEY]);
	...
	it('should keep both entries reachable through the shared meta key', async () => {
		...
		expect(members.sort()).toEqual(['cache:team-detail', 'cache:teams-collection']);
	});
