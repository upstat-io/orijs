...
export class RedisWsProvider implements WebSocketProvider {
  ...
  public subscribe(socketId: string, topic: string): void {
    ...
    if (wasEmpty && this.subscriber) {
      ...
      if (
        !this.redisSubscriptions.has(channel) &&
        !this.pendingSubscriptions.has(channel)
      ) {
  ...
  private resubscribeAll(): void {
    if (
      !this.subscriber ||
      !this.started ||
      this.redisSubscriptions.size === 0
    ) {
      return;
    }

    const channels = Array.from(this.redisSubscriptions);
    this.logger.info("Redis subscriber reconnected, resubscribing", {
      channelCount: channels.length,
    });
 ...
 * ```
 */
export function createRedisWsProvider(
  options: RedisWsProviderOptions,
): RedisWsProvider {
