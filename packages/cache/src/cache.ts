 ...
 */
function createFactoryContext<T>(
  staleValue: T | undefined,
  staleAge: number | undefined,
): FactoryContext<T> {
  return {
    skip(): never {
      throw SKIP_SENTINEL;
...
export class CacheService {
  ...
  constructor(
    ...
    this.defaultGraceSeconds = options.defaultGrace
  ...
  async getOrSet<T, TParams extends object>(
    ...
    return this.singleflight.do(cacheKey, async () => {
      ...
      const isStale =
        entry && entry.graceExpiresAt && entry.graceExpiresAt > now;
   ...
   */
  async delete<TParams extends object>(
    config: CacheConfig<TParams>,
    params: TParams,
  ): Promise<boolean> {
    const cacheKey = generateCacheKey(config, params);
  ...
  async invalidate<TEntityName extends string, TParams extends object>(
    ...
    if (cascade && hasMetaSupport(this.provider)) {
      ...
      const invalidationTags = getEntityInvalidationTags(
        entityType,
    ...
    const cacheKeys = this.generateCacheKeysForEntity(
      entityType,
      params as Record<string, unknown>,
    );
  ...
  async invalidateMany<TEntityName extends string, TParams extends object>(
    ...
    if (cascade && hasMetaSupport(this.provider)) {
      ...
      for (const inv of invalidations) {
        ...
        const tags = getEntityInvalidationTags(
          inv.entityType,
          inv.params as Record<string, unknown>,
        );
    ...
    for (const inv of invalidations) {
      allCacheKeys.push(
        ...this.generateCacheKeysForEntity(
          inv.entityType,
          inv.params as Record<string, unknown>,
        ),
      );
   ...
   */
  private trackConfig<TParams extends object>(
    config: CacheConfig<TParams>,
  ): void {
   ...
   */
  public registerConfig<TParams extends object>(
    config: CacheConfig<TParams>,
  ): void {
    this.trackConfig(config);
  }
   ...
   */
  private generateCacheKeysForEntity(
    entityType: string,
   ...
   */
  private generateMetaKeys<TParams extends object>(
    config: CacheConfig<TParams>,
    params: TParams,
  ): string[] {
