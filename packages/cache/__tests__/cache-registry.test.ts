...
describe("CacheRegistry", () => {
  ...
  describe("validateNoCycles()", () => {
    ...
    it("should detect direct cycle (A -> A) at build time via self-dependency guard", () => {
      ...
      expect(() =>
        Cache.for("User").ttl("1h").dependsOn("User").build(),
      ).toThrow(
    ...
    it("should detect indirect cycle (A -> B -> A) via validateNoCycles() at register time", () => {
      ...
      const monitorConfig = Cache.for("Monitor")
        .ttl("5m")
        .dependsOn("Alert")
        .build();

      // Alert depends on Monitor (creates cycle)
      const alertConfig = Cache.for("Alert")
        .ttl("5m")
        .dependsOn("Monitor")
        .build();
      ...
      expect(() => cacheRegistry.validateNoCycles()).toThrow(
        /Circular cache dependency detected/,
      );
    });
  });
