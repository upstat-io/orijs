...
describe("CacheService invalidate with InMemory provider", () => {
  ...
  test("should delete cached entry via invalidate()", async () => {
    ...
    // Invalidate
    const deleted = await cacheService.invalidate("User", params, {
      cascade: false,
    });
  ...
  test("should delete multiple entries via invalidateMany()", async () => {
    ...
    await cacheService.getOrSet(UserCache, params1, async () => ({
      name: "Alice",
    }));
    await cacheService.getOrSet(UserCache, params2, async () => ({
      name: "Bob",
    }));
  ...
  test("should return 0 when params do not match any tracked config", async () => {
    ...
    const deleted = await cacheService.invalidate(
      "User",
      { accountUuid: "acc-1", userUuid: "usr-1" },
      { cascade: false },
    );
  ...
  test("should work after registerConfig() for invalidation-only instances", async () => {
    ...
    // Populate cache
    await freshService.getOrSet(UserCache, params, async () => ({
      name: "Alice",
    }));
    ...
    const deleted = await invalidateOnlyService.invalidate("User", params, {
      cascade: false,
    });
