...
describe("dependsOn", () => {
  ...
  describe("explicit override", () => {
    ...
    it("should allow empty params array", () => {
      ...
      const config = Cache.for<{ accountUuid: string }>("User")
        .ttl("1h")
        .dependsOn("Account", [])
        .build();
...
describe("EntityDef input", () => {
  describe("Cache.for(EntityDef)", () => {
    ...
    it("should work interchangeably with string input", () => {
      ...
      const configFromEntityDef = Cache.for<TestParams>(Entities.User)
        .ttl("1h")
        .build();
...
describe("functional tests", () => {
  ...
  it("should work with .use() composition in registry", () => {
    ...
    expect(MonitorSettingsCache.dependsOn.Monitor).toEqual([
      "accountUuid",
      "projectUuid",
      "monitorUuid",
    ]);
  ...
  it("should build cache config using factory-based workflow", () => {
    ...
    const MonitorCache = Cache.for<TestParams>(Entities.Monitor)
      .ttl("5m")
      .build();
    ...
    expect(MonitorCache.scope).toBe("project");
    expect(MonitorCache.params).toEqual([
      "accountUuid",
      "projectUuid",
      "monitorUuid",
    ]);
    ...
    expect(MonitorCache.dependsOn.Project).toEqual([
      "accountUuid",
      "projectUuid",
    ]);
