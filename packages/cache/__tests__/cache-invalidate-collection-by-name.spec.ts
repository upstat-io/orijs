...
describe('invalidate(Collection, params) — direct collection invalidation', () => {
	...
	beforeAll(() => {
		...
		redisCacheProvider = new RedisCacheProvider({ connection: { host: c.host, port: c.port, db: c.db } });
	...
	it('removes the entry when the caller threads a wider request-scoped object through both calls', async () => {
		...
		await cacheService.getOrSet(TeamsCollectionCache, wide as unknown as CollectionParams, factory);
		await cacheService.getOrSet(TeamsCollectionCache, wide as unknown as CollectionParams, factory);
		expect(calls).toBe(1);
	...
	it('removes the entry when a hit-only service invalidates with a wider params object', async () => {
		...
		await restarted.getOrSet(TeamsCollectionCache, wide as unknown as CollectionParams, factory);
		expect(calls).toBe(1);
