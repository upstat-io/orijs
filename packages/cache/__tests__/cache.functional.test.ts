...
describe("CacheService (functional)", () => {
  ...
  describe("getOrSet - cache-aside pattern", () => {
    ...
    it("should use different cache keys for different params", async () => {
      ...
      const user1 = await cacheService.getOrSet<
        User,
        { accountUuid: string; userUuid: string }
      >(
        UserCache,
  ...
  describe("fail() behavior", () => {
    it("should not cache result when factory calls ctx.fail() and no stale value", async () => {
      ...
      await expect(
        cacheService.getOrSet<User, typeof params>(
          UserCache,
          params,
          async (ctx) => {
            factoryCallCount++;
  ...
  describe("cacheNull option", () => {
    it("should not cache null by default", async () => {
      ...
      await cacheService.getOrSet<User | null, typeof params>(
        UserCache,
        params,
        async () => {
          factoryCallCount++;
    ...
    it("should cache null when cacheNull is enabled", async () => {
      ...
      // First call
      const result1 = await cacheService.getOrSet<User | null, typeof params>(
        UserCache,
        params,
        async () => {
          factoryCallCount++;
      ...
      const result2 = await cacheService.getOrSet<User | null, typeof params>(
        UserCache,
        params,
        async () => {
          factoryCallCount++;
  ...
  describe("direct get/set/delete", () => {
    it("should set and get value directly", async () => {
      ...
      const result = await cacheService.get<User, typeof params>(
        UserCache,
        params,
      );
    ...
    it("should delete cached value", async () => {
      ...
      await cacheService.set(UserCache, params, user);
      expect(
        await cacheService.get<User, typeof params>(UserCache, params),
      ).toEqual(user);
  ...
  describe("invalidation", () => {
    it("should invalidate cache entries by entity type", async () => {
      ...
      await cacheService.set(UserCache, params1, {
        uuid: "user-1",
        name: "User 1",
        email: "1@test.com",
      });
      await cacheService.set(UserCache, params2, {
        uuid: "user-2",
        name: "User 2",
        email: "2@test.com",
      });
      ...
      expect(
        await cacheService.get<User, typeof params1>(UserCache, params1),
      ).toBeDefined();
      expect(
        await cacheService.get<User, typeof params2>(UserCache, params2),
      ).toBeDefined();
    ...
    it("should invalidate entries based on dependencies", async () => {
      ...
      expect(
        await cacheService.get<Monitor, typeof params>(MonitorCache, params),
      ).toBeDefined();
    ...
    it("should not affect entries in different scopes", async () => {
      ...
      await cacheService.set(MonitorCache, params1, {
        uuid: "mon-1",
        name: "Mon 1",
        url: "http://1",
      });
      await cacheService.set(MonitorCache, params2, {
        uuid: "mon-2",
        name: "Mon 2",
        url: "http://2",
      });
      ...
      await cacheService.invalidate("Project", {
        accountUuid,
        projectUuid: params1.projectUuid,
      });

      // Project 1 monitor should be cleared
      expect(
        await cacheService.get<Monitor, typeof params1>(MonitorCache, params1),
      ).toBeUndefined();
      // Project 2 monitor should still be cached
      expect(
        await cacheService.get<Monitor, typeof params2>(MonitorCache, params2),
      ).toBeDefined();
    });
  });
  ...
  describe("dependency-based cascade with fewer params", () => {
    ...
    it("should cascade to dependent with fewer params", async () => {
      ...
      const Cache = createCacheBuilder(registry);
      const UserCache = Cache.for<{ accountUuid: string; userId: string }>(
        "User",
      )
        .ttl("1h")
        .build();
      ...
      await cacheService.set(UserListCache, { accountUuid }, [
        { name: "User 1" },
      ]);

      expect(
        await cacheService.get(UserCache, { accountUuid, userId }),
      ).toBeDefined();
      expect(
        await cacheService.get(UserListCache, { accountUuid }),
      ).toBeDefined();
    ...
    it("should not cascade to different scope", async () => {
      ...
      await cacheService.set(UserListCache, { accountUuid: account1 }, [
        { name: "List 1" },
      ]);
      await cacheService.set(UserListCache, { accountUuid: account2 }, [
        { name: "List 2" },
      ]);
      ...
      expect(
        await cacheService.get(UserListCache, { accountUuid: account1 }),
      ).toBeUndefined();
      expect(
        await cacheService.get(UserListCache, { accountUuid: account2 }),
      ).toBeDefined();
    });
    ...
    it("should cascade to multiple dependents", async () => {
      ...
      const ItemListCache = Cache.for<{
        ...
        .dependsOn("Item")
        .build();
      const ItemCountCache = Cache.for<{
        accountUuid: string;
        projectUuid: string;
      }>("ItemCount")
        .ttl("1h")
      ...
      await cacheService.set(
        ItemCache,
        { accountUuid, projectUuid, itemId },
        { name: "Item 1" },
      );
      await cacheService.set(ItemListCache, { accountUuid, projectUuid }, [
        { name: "Item 1" },
      ]);
      await cacheService.set(
        ItemCountCache,
      ...
      await cacheService.invalidate("Item", {
        accountUuid,
        projectUuid,
        itemId,
      });
      ...
      expect(
        await cacheService.get(ItemCache, { accountUuid, projectUuid, itemId }),
      ).toBeUndefined();
      expect(
        await cacheService.get(ItemListCache, { accountUuid, projectUuid }),
      ).toBeUndefined();
      expect(
        await cacheService.get(ItemCountCache, { accountUuid, projectUuid }),
      ).toBeUndefined();
    });
  });
  ...
  describe("invalidateMany", () => {
    it("should invalidate multiple entities at once", async () => {
      ...
      await cacheService.set(UserCache, acc1User, {
        uuid: "user-1",
        name: "U1",
        email: "1@test.com",
      });
      await cacheService.set(UserCache, acc2User, {
        uuid: "user-2",
        name: "U2",
        email: "2@test.com",
      });
      await cacheService.set(UserCache, acc3User, {
        uuid: "user-3",
        name: "U3",
        email: "3@test.com",
      });
      ...
      expect(
        await cacheService.get<User, typeof acc1User>(UserCache, acc1User),
      ).toBeUndefined();
      expect(
        await cacheService.get<User, typeof acc2User>(UserCache, acc2User),
      ).toBeUndefined();
      expect(
        await cacheService.get<User, typeof acc3User>(UserCache, acc3User),
      ).toBeDefined();
    });
  });
  ...
  describe("grace period stale value", () => {
    it("should pass stale value and staleAge to factory during grace period", async () => {
      ...
      try {
        await cacheService.getOrSet<User, typeof params>(
          GraceCache,
          params,
          async () => originalUser,
        );
  ...
  describe("TTL expiration", () => {
    it("should expire cache after TTL", async () => {
      ...
      expect(
        await cacheService.get<User, typeof params>(ShortCache, params),
      ).toBeDefined();
  ...
  describe("timeout behavior", () => {
    ...
    it("should return stale value when fetch times out during grace period", async () => {
      ...
      const TimeoutWithGraceCache = Cache.for("User")
        .ttl(0.2)
        .grace(1)
        .timeout(0.05)
        .build();
    ...
    it("should use 1 second default timeout", async () => {
      ...
      await expect(
        cacheService.getOrSet<User, typeof slowParams>(
          DefaultTimeoutCache,
          slowParams,
          async () => {
  ...
  describe("tag-based invalidation", () => {
    ...
    it("should invalidate tagged cache when entity with matching invalidationTags is invalidated", async () => {
      ...
      const UserCache = Cache.for<{ accountUuid: string; fbAuthUid: string }>(
        "User",
      )
        .ttl("1h")
      ...
      await cacheService.set(
        UserCache,
        { accountUuid, fbAuthUid },
        { name: "Test User" },
      );

      // Verify both are cached
      expect(
        await cacheService.get<{ accountUuid: string }, { fbAuthUid: string }>(
          UserAuthCache,
          { fbAuthUid },
        ),
      ).toBeDefined();
      expect(
        await cacheService.get<
          { name: string },
          { accountUuid: string; fbAuthUid: string }
        >(UserCache, {
          accountUuid,
          fbAuthUid,
        }),
    ...
    it("should handle multiple tags on single cache", async () => {
      ...
      const MonitorCacheConfig = Cache.for<{
        cacheKey: string;
        projectUuid: string;
        monitorUuid: string;
      }>("MonitorCache")
      ...
      await cacheService.set(
        MonitorCacheConfig,
        { cacheKey, projectUuid, monitorUuid },
        { data: "test" },
      );

      // Verify it's cached
      expect(
        await cacheService.get(MonitorCacheConfig, {
          cacheKey,
          projectUuid,
          monitorUuid,
        }),
    ...
    it("should handle empty invalidationTags array (no-op)", async () => {
      ...
      const Cache = createCacheBuilder(registry);
      const EmptyTagsCache = Cache.for<{
        accountUuid: string;
        itemUuid: string;
      }>("EmptyTags")
        .ttl("1h")
      ...
      expect(
        await cacheService.get(EmptyTagsCache, { accountUuid, itemUuid }),
      ).toBeUndefined();
    ...
    it("should work for cross-scope invalidation (UserAuth scenario)", async () => {
      ...
      await cacheService.set(
        UserAuthCache,
        { fbAuthUid },
        { fbAuthUid, accountUuid },
      );

      // Verify UserAuth is cached
      const cachedUserAuth = await cacheService.get<
        UserAuthData,
        { fbAuthUid: string }
      >(UserAuthCache, {
        fbAuthUid,
