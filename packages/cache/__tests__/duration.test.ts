...
describe("parseDuration", () => {
  describe("numeric input (passthrough)", () => {
    ...
    it("should throw for durations exceeding 365 days", () => {
      ...
      expect(() => parseDuration(oneYearPlusOne)).toThrow(
        "Duration exceeds maximum of 365 days",
      );
    });
  ...
  describe("error cases", () => {
    it("should throw for empty string", () => {
      expect(() => parseDuration("" as Duration)).toThrow(
        "Invalid duration format",
      );
    });

    it("should throw for invalid unit", () => {
      expect(() => parseDuration("5x" as Duration)).toThrow(
        "Invalid duration format: 5x",
      );
      expect(() => parseDuration("10w" as Duration)).toThrow(
        "Invalid duration format: 10w",
      );
    });

    it("should throw for decimal values", () => {
      expect(() => parseDuration("1.5h" as Duration)).toThrow(
        "Invalid duration format",
      );
      expect(() => parseDuration("2.5m" as Duration)).toThrow(
        "Invalid duration format",
      );
    });

    it("should throw for missing value", () => {
      expect(() => parseDuration("m" as Duration)).toThrow(
        "Invalid duration format",
      );
      expect(() => parseDuration("h" as Duration)).toThrow(
        "Invalid duration format",
      );
    });

    it("should throw for spaces", () => {
      expect(() => parseDuration("5 m" as Duration)).toThrow(
        "Invalid duration format",
      );
      expect(() => parseDuration(" 5m" as Duration)).toThrow(
        "Invalid duration format",
      );
    });

    it("should throw for multiple units", () => {
      expect(() => parseDuration("1h30m" as Duration)).toThrow(
        "Invalid duration format",
      );
    });

    it("should throw for string durations exceeding 365 days", () => {
      expect(() => parseDuration("366d" as Duration)).toThrow(
        "Duration exceeds maximum of 365 days",
      );
      expect(() => parseDuration("400d" as Duration)).toThrow(
        "Duration exceeds maximum of 365 days",
      );
    });
