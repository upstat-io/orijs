...
export type ConfigProviderInput =
  ConfigProvider | ConfigProviderConstructor | ConfigProviderFactory;

/**
 * Type guard to check if input is a factory object with create() method.
 */
function isConfigProviderFactory(
  input: ConfigProviderInput,
): input is ConfigProviderFactory {
  return (
    typeof input === "object" &&
    input !== null &&
    "create" in input &&
 ...
 */
function isConfigProviderConstructor(
  input: ConfigProviderInput,
): input is ConfigProviderConstructor {
...
export class NamespacedConfigBuilder {
  ...
  add(namespace: string, provider: ConfigProviderInput): this {
    if (namespace === "env") {
      throw new Error(
        'Cannot override "env" namespace - it is reserved for environment variables',
      );
    }
  ...
  async validate<T = NamespacedConfigResult>(): Promise<T & ConfigProvider> {
    ...
    for (const namespace of Object.keys(this.expectedKeys)) {
      if (namespace !== "env" && !this.providers.has(namespace)) {
        throw new Error(
          `Namespace "${namespace}" in expectKeys was not added via .add()`,
        );
      }
    }
    ...
    return new Proxy(result, {
      get(target, prop: string) {
        ...
        if (prop === "getRequired")
          return configProvider.getRequired.bind(configProvider);
        if (prop === "loadKeys")
          return configProvider.loadKeys.bind(configProvider);
