...
describe("createMockSql", () => {
  ...
  describe("template mode", () => {
    ...
    it("should capture strings and values with interpolations", () => {
      ...
      const id = 42;
      const name = "test";
      void mock`SELECT * FROM users WHERE id = ${id} AND name = ${name}`;
    ...
    it("should overwrite lastCall on subsequent calls", () => {
      const mock = createMockSql();
      void mock`first query`;
      void mock`second query`;
    ...
    it("should handle various value types", () => {
      ...
      void mock`SELECT ${42} ${true} ${null} ${date} ${obj}`;
  ...
  describe("identifier mode", () => {
    ...
    it("should preserve exact identifier name", () => {
      ...
      expect(mock("column_name")).toEqual({
        __isIdentifier: true,
        name: "column_name",
      });
  ...
  describe("isIdentifierMarker", () => {
    ...
    it("should return false for objects without __isIdentifier", () => {
      ...
      expect(
        mock.isIdentifierMarker({ __isIdentifier: false, name: "test" }),
      ).toBe(false);
    });
  ...
  describe("integration with createOriSql pattern", () => {
    it("should work with identifier markers passed as template values", () => {
      ...
      void mock`SELECT ${columnMarker} FROM ${tableMarker} WHERE id = ${42}`;
      ...
      expect((mock.lastCall!.values[0] as IdentifierMarker).name).toBe(
        "column",
      );
