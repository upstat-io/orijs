...
describe("createOriSql", () => {
  describe("identifier conversion", () => {
    it("should convert identifier marker to bunSql() call", () => {
      ...
      void oriSql`SELECT ${["uuid"]} FROM account`;
      ...
      expect(mockSql.isIdentifierMarker(mockSql.lastCall!.values[0])).toBe(
        true,
      );
    ...
    it("should convert multiple identifiers", () => {
      ...
      void oriSql`SELECT ${["uuid"]}, ${["name"]} FROM ${["account"]}`;

      expect(mockSql.lastCall!.strings).toEqual([
        "SELECT ",
        ", ",
        " FROM ",
        "",
  ...
  describe("parameter passthrough", () => {
    it("should pass regular values through unchanged", () => {
      ...
      void oriSql`SELECT * FROM account WHERE id = ${42}`;
    ...
    it("should handle multiple parameters", () => {
      ...
      void oriSql`SELECT * FROM account WHERE name = ${"Test"} AND id = ${42}`;
    ...
    it("should handle null and undefined", () => {
      ...
      void oriSql`UPDATE account SET a = ${null}, b = ${undefined}`;
    ...
    it("should treat multi-element array as parameter", () => {
      ...
      void oriSql`SELECT * FROM account WHERE tags = ${["a", "b"]}`;
    ...
    it("should treat empty array as parameter", () => {
      ...
      void oriSql`SELECT * FROM account WHERE tags = ${[]}`;
    ...
    it("should treat array with non-string element as parameter", () => {
      ...
      void oriSql`SELECT * FROM account WHERE id = ${[123]}`;
  ...
  describe("mixed identifiers and parameters", () => {
    it("should handle identifiers and parameters together", () => {
      ...
      void oriSql`SELECT ${["uuid"]} FROM ${["account"]} WHERE ${["id"]} = ${42}`;
      ...
      expect(mockSql.isIdentifierMarker(mockSql.lastCall!.values[1])).toBe(
        true,
      );
      expect(mockSql.isIdentifierMarker(mockSql.lastCall!.values[2])).toBe(
        true,
      );
    ...
    it("should handle complex query with joins", () => {
      ...
      const identifiers = mockSql.lastCall!.values.filter((v) =>
        mockSql.isIdentifierMarker(v),
      );
      expect(identifiers).toHaveLength(7); // uuid, name, user, account, id, account_id, id

      // The parameter value should be passed through
      const params = mockSql.lastCall!.values.filter(
        (v) => !mockSql.isIdentifierMarker(v),
      );
  ...
  describe("edge cases", () => {
    ...
    it("should handle query with only identifiers", () => {
      ...
      void oriSql`SELECT ${["a"]}, ${["b"]} FROM ${["c"]}`;

      expect(mockSql.lastCall!.values).toHaveLength(3);
      expect(
        mockSql.lastCall!.values.every((v) => mockSql.isIdentifierMarker(v)),
      ).toBe(true);
    });
    ...
    it("should handle query with only parameters", () => {
      ...
      void oriSql`SELECT * FROM account WHERE id = ${1} AND name = ${"test"}`;
...
WHERE ${["id"]} = ${1}`;
  ...
  describe("SQL injection safety", () => {
    it("should parameterize values preventing SQL injection", () => {
      ...
      void oriSql`SELECT * FROM account WHERE name = ${userInput}`;
    ...
    it("should pass identifiers to Bun sql() for validation", () => {
      ...
      void oriSql`SELECT ${[maliciousIdentifier]} FROM account`;
      ...
      expect((mockSql.lastCall!.values[0] as any).name).toBe(
        maliciousIdentifier,
      );
    });
  });
  ...
  describe("error handling", () => {
    it("should propagate errors from underlying sql function", () => {
      ...
      expect(() => oriSql`SELECT * FROM account`).toThrow(
