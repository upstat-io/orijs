...
export interface ApplicationOptions {
  ...
  socketRoutingCoordinatorFactory?: (
    container: Container,
    logger: Logger,
...
> extends OriApplication {
  ...
  consumer(
    consumerClass: Constructor<
      IWorkflowConsumer<TData, TResult, NoInfer<TSteps>>
    >,
    deps?: InjectionToken[],
  ): OriApplication;
}
...
export class OriApplication<TSocket extends SocketEmitter = SocketEmitter> {
  ...
  private deferredExtensions: Array<
    (app: OriApplication<any>) => OriApplication<any>
  > = [];
  ...
  private websocketEmitterClass: SocketEmitterConstructor<SocketEmitter> | null =
    null;
  private websocketEmitterInstance: SocketEmitter | null = null;
  private _websocketUpgrade:
    ((request: Request) => Promise<unknown | null> | unknown | null) | null =
    null;
  ...
  constructor(options?: ApplicationOptions) {
    ...
    this.container =
      options?.container ?? new Container({ logger: this.appLogger });
  ...
  public config(
    factoryOrProvider:
      ConfigProvider | ((app: OriApplication<TSocket>) => Promise<void>),
  ): this {
   ...
   */
  public provider<T extends new () => unknown>(
    service: T,
  ...
  public providerWithTokens<T extends Constructor>(
    ...
    this.providerCoordinator.addProvider(
      service,
      deps as Constructor[],
      options?.eager,
    );
   ...
   */
  public use(
    extension: (app: OriApplication<TSocket>) => OriApplication<TSocket>,
  ): this {
      ...
      get: (target, prop, receiver) => {
        if (prop === "consumer") {
          return (
            ...
            target.eventCoordinator.addEventConsumer(
              definition,
              consumerClass,
              deps,
            );
      ...
      get: (target, prop, receiver) => {
        if (prop === "consumer") {
          return (
            ...
            target.workflowCoordinator.addWorkflowConsumer(
              definition,
              consumerClass,
              deps,
            );
   ...
   */
  public websocket<
    TEmitter extends SocketEmitter = SocketEmitter,
    TData = unknown,
  >(
    ...
    const wsProvider =
      provider ??
      new InProcWsProvider({ logger: this.appLogger.child("WebSocket") });
    ...
    this.websocketEmitterClass = (options?.emitter ??
      null) as SocketEmitterConstructor<SocketEmitter> | null;
   ...
   * ```
   */
  public onWebSocket<TData = unknown>(
    handlers: WebSocketHandlers<TData>,
  ): this {
  ...
  >(): TEmitter {
    ...
    if (this.websocketEmitterInstance === null) {
      if (this.websocketEmitterClass) {
        this.websocketEmitterInstance = new this.websocketEmitterClass(
   ...
   */
  public controller<T extends new () => unknown>(
    path: string,
    controller: T & ControllerClass,
  ): this;
  /**
   ...
   * @param deps - Array of dependency classes (must match constructor parameter types and order)
   */
  public controller<T extends ControllerClass>(
    path: string,
    controller: T,
    deps: ConstructorDeps<T>,
  ): this;
   ...
   */
  public socketRouter<T extends new () => unknown>(
    router: T & SocketRouterClass,
  ): this;
  /**
   ...
   * ```
   */
  public socketRouter<T extends SocketRouterClass>(
    router: T,
    deps: ConstructorDeps<T>,
  ): this;
  public socketRouter<T extends SocketRouterClass>(
  ...
  private async startListening(
    ...
    if (this.socketRoutingCoordinator.hasRouters()) {
      this.socketRoutingCoordinator.initialize(
        this._context,
    ...
    if (this.websocketProvider) {
      this.appLogger.debug("Creating server with WebSocket support", {
        path: this._websocketPath,
      });
    ...
    if (this.websocketProvider) {
      ...
      this.appLogger.info(
        `WebSocket enabled at: ws://localhost:${port}${this._websocketPath}`,
      );
    }
  ...
  private async handleFetch(
    ...
    if (this.websocketProvider) {
      ...
      if (
        url.pathname === this._websocketPath &&
  ...
  private buildStaticCorsHeaders(): Record<string, string> {
    ...
    const headers: Record<string, string> = {
      ...
      "Access-Control-Allow-Headers": (
        this._corsConfig.allowedHeaders ?? [
          "Content-Type",
          "Authorization",
          "X-Firebase-AppCheck",
        ]
    ...
    if (this._corsConfig.exposedHeaders?.length) {
      headers["Access-Control-Expose-Headers"] =
        this._corsConfig.exposedHeaders.join(", ");
    }
  ...
  private async handleWebSocketUpgrade(
    ...
    try {
      ...
      if (this._websocketUpgrade) {
        ...
        try {
          data = await Promise.race([
            this._websocketUpgrade(request),
            timeoutPromise,
          ]);
        } finally {
      ...
      this.appLogger.debug("WebSocket upgrade successful", {
        socketId,
        correlationId: upgradeCorrelationId,
      });
      const socketData = {
  ...
  private buildWebSocketHandlers() {
     ...
     */
    const wrapWebSocket = (
      ws: WebSocketConnection<unknown>,
      ...
      open: (ws: WebSocketConnection<unknown>) => {
        ...
        if (hasSocketRouters) {
          socketRouting
            ...
            .catch((error) => {
              logger.error("Socket controller connection handler error", {
                error,
              });
              ...
              socketRouting
                ...
                .then((handled) => {
                  if (!handled) {
                    ...
                    const wrappedWs = wrapWebSocket(ws);
                    safeCall("message", () =>
                      handlers?.message?.(wrappedWs, message),
                    );
                  }
                })
                .catch((error) => {
                  logger.error("Socket controller message handler error", {
                    error,
                  });
                  ws.send(
                    JSON.stringify({
