...
> implements BaseContext {
  ...
  get workflows(): WorkflowExecutor {
    if (!this.workflowExecutor) {
      throw new Error(
        "Workflows not configured. Call .workflows(registry) when creating the application.",
      );
    }
  ...
  public setSocketEmitterGetter<T extends SocketEmitter>(
    getter: () => T,
  ): void {
  ...
  public onStartup(hook: LifecycleHook): void {
    if (
      ...
      this.currentPhase !== "bootstrapped"
    ) {
