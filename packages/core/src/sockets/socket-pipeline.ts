...
export class SocketPipeline<TSocket extends SocketEmitter = SocketEmitter> {
  ...
  constructor(
    ...
    this.contextFactory = new SocketContextFactory<TSocket>(
      appContext,
      loggerOptions,
    );
  }
  ...
  public async handleMessage<TData>(
    ...
    await runWithContext(
      { log: this.appLogger, correlationId: requestCorrelationId, trace },
      async () => {
        try {
          ...
          for (const [key, value] of Object.entries(connectionState)) {
            ctx.set(
              key as keyof typeof ctx.state,
              value as (typeof ctx.state)[keyof typeof ctx.state],
            );
          }
