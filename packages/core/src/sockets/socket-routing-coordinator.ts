 ...
 */
export class SocketRoutingCoordinator<
...
> {
  ...
  public initialize(
    ...
    this.pipeline = new SocketPipeline<TSocket>(
      this.container,
      appContext,
      this.appLogger,
      loggerOptions,
    ...
    for (const config of this.routerConfigs) {
      ...
      for (const route of routes) {
        ...
        this.compiledRoutes.set(
          route.messageType,
          this.pipeline.compileRoute(route),
        );
      }
    }
  }
   ...
   */
  public async handleConnection<TData>(
    ...
    const ctx = await this.pipeline.runConnectionGuards(
      ws,
