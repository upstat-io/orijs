...
export class RoutingCoordinator {
  ...
  public registerControllers(): void {
    for (const { path: controllerPath, controller, deps } of this.controllers) {
      ...
      this.container.register(
        controller,
        deps as ConstructorDeps<typeof controller>,
      );
      ...
      const routePaths = routes.map(
        (r) => `${r.method} ${controllerPath}${r.path === "/" ? "" : r.path}`,
      );
      this.logger.debug(
        ...
        for (const route of pathRoutes) {
          if (route.handler instanceof Response) {
            const staticResponse = corsHeaders
              ? this.addCorsToStaticResponse(route.handler, corsHeaders)
              : route.handler;
            methodHandlers[route.method] = () =>
              new Response(staticResponse.body, {
   ...
   */
  private addCorsToStaticResponse(
    response: Response,
