...
export class WorkflowCoordinator {
  ...
  private instantiatedConsumers: Map<string, InstantiatedWorkflowConsumer> =
    new Map();

  constructor(
    private readonly logger: Logger,
    private readonly container: Container,
    private readonly defaultProviderFactory: WorkflowProviderFactory = () =>
      new InProcessWorkflowProvider(),
  ) {}
  ...
  public setProvider(provider: WorkflowProvider): void {
    if (this.workflowProvider) {
      this.logger.warn(
        "Workflow provider already set, replacing with new provider",
      );
    }
   ...
   */
  public registerWorkflowDefinition<TData, TResult, TSteps>(
    ...
    if (this.workflowDefinitions.has(workflowName)) {
      throw new Error(
        `Duplicate workflow registration: "${workflowName}" is already registered`,
      );
    }

    this.workflowDefinitions.set(
      workflowName,
      definition as WorkflowDefinition<unknown, unknown>,
    );
  ...
  public addWorkflowConsumer<TData, TResult, TSteps>(
    ...
    this.pendingConsumers.push({
      ...
      consumerClass: consumerClass as Constructor<
        IWorkflowConsumer<unknown, unknown>
      >,
      deps,
    });
  }
  ...
  public registerConsumers(): void {
    ...
    if (!this.workflowProvider) {
      if (
        ...
        this.workflowDefinitions.size > 0
      ) {
        this.logger.debug(
          "No workflow provider configured, using InProcessWorkflowProvider",
        );
    ...
    for (const { definition, consumerClass, deps } of this.pendingConsumers) {
      ...
      const hasConfigure =
        typeof (consumer as { configure?: unknown }).configure === "function";
      const hasStepsProperty =
        consumer.steps && Object.keys(consumer.steps).length > 0;
      const hasStepsOnDefinition =
        definition.stepGroups && definition.stepGroups.length > 0;
      ...
      if (hasStepsOnDefinition && !hasStepsProperty) {
        const stepNames = definition.stepGroups.flatMap((g) =>
      ...
      if (this.workflowProvider.registerDefinitionConsumer) {
        ...
        const handler = async (
          ...
          if (!Value.Check(definition.dataSchema, data)) {
            ...
            throw new Error(
              `Invalid workflow data: ${errors.map((e) => e.message).join(", ")}`,
            );
          }
          ...
          const correlationId =
            (metaRecord.correlationId as string | undefined) ??
            crypto.randomUUID();
      ...
      this.logger.info(
        `Workflow Consumer Registered -> [${workflowName}] [${consumerClass.name}]`,
      );
    }
  ...
  public getConsumer(
    workflowName: string,
  ): InstantiatedWorkflowConsumer | undefined {
  ...
  public getWorkflowDefinition(
    workflowName: string,
  ): WorkflowDefinition<unknown, unknown> | undefined {
    ...
    return {
      async execute<TData, TResult>(
        ...
        if (!isWorkflowDefinition(workflow)) {
          throw new Error(
            "Expected WorkflowDefinition. Class-based workflows are no longer supported.",
          );
        }
        ...
        if (registered && provider && options?.id) {
          if (!Value.Check(workflow.dataSchema, data)) {
            ...
            const errorDetails = errors
              ...
              .join(", ");
            throw new Error(
              `Workflow "${workflowName}" data validation failed: ${errorDetails}`,
            );
          }
        ...
        // Extract or generate correlationId for distributed tracing
        const correlationId =
          (meta.correlationId as string | undefined) ?? crypto.randomUUID();
            ...
            if (step.handler.rollback) {
              try {
                ...
                workflowLogger.debug(
                  `Rollback completed for step "${step.name}"`,
                );
              } catch (rollbackError) {
          ...
          for (const group of definition.stepGroups) {
            if (group.type === "sequential") {
              ...
              for (const stepDef of group.definitions) {
                ...
                if (!handler) {
                  throw new Error(
                    `Workflow "${workflowName}": no handler for step "${stepDef.name}"`,
                  );
                }
                ...
                try {
                  ...
                  if (!Value.Check(stepDef.outputSchema, stepResult)) {
                    const errors = [
                      ...Value.Errors(stepDef.outputSchema, stepResult),
                    ];
                    const errorDetails = errors
                      .map((e) => `${e.path}: ${e.message}`)
                      .join(", ");
              ...
              const parallelPromises = group.definitions.map(
                async (stepDef) => {
                  const handler = stepHandlers[stepDef.name];
                  if (!handler) {
                    throw new Error(
              ...
              const fulfilled: {
                name: string;
                result: unknown;
                handler: (typeof stepHandlers)[string];
              }[] = [];
        ...
        const result = (await consumer.onComplete(
        ...
        if (!Value.Check(definition.resultSchema, result)) {
          ...
          const errorDetails = errors
            ...
            .join(", ");
          throw new Error(
            `Workflow "${workflowName}" result validation failed: ${errorDetails}`,
          );
        }
