...
export class RequestBoundEventEmitter implements EventEmitter {
   ...
   */
  public emit<TPayload, TResponse>(
    event: EventDefinition<TPayload, TResponse>,
    payload: TPayload,
    options?: EventEmitOptions,
  ): Promise<TResponse> {
    // Deliberately NOT declared `async`: an `async` method always wraps
    // its `return`ed value in a NEW promise distinct from the one
    // returned, so a `.catch()` attached below to mark a promise
    // "handled" (for Node/Bun's unhandled-rejection tracking) would mark
    // the WRONG promise — the async wrapper's own promise, which the
    // caller actually receives, would remain a separate, unhandled
    // object. Returning promises directly keeps the object identity the
    // caller observes (`ctx.events.emit(...)`) the same object this
    // method marks handled via `logRejectionWithContext`.
    const provider = this.eventCoordinator.getProvider();
    if (!provider) {
      return this.logRejectionWithContext(
        event.name,
        Promise.reject(
          new Error(
    ...
    if (!Value.Check(event.dataSchema, payload)) {
      ...
      return this.logRejectionWithContext(
        ...
        Promise.reject(
          new Error(
            `Event "${event.name}" payload validation failed: ${errorDetails}`,
          ),
        ),
      );
    }
    ...
    return this.logRejectionWithContext(event.name, subscription.toPromise());
  }

  /**
   * Attaches a request-context-carrying, non-propagating rejection logger
   * to `promise` as a side effect, then returns the SAME promise instance.
   *
   * Fire-and-forget callers (`void ctx.events.emit(...)`) never observe a
   * rejection directly, so without this handler the failure would only
   * reach the process-level `unhandledRejection` listener, which logs
   * solely `error.message` + `stack` with no request context
   * (correlationId, event name, accountUuid/projectUuid carried in the
   * payload). Attaching `.catch()` here marks `promise` handled for
   * Node/Bun's unhandled-rejection tracking BEFORE returning it, so an
   * unawaited emit no longer surfaces without context. Returning the exact
   * same instance (never a `.then()`/`.catch()` derivative) means an
   * awaiting caller still observes the original rejection unchanged.
   */
  private logRejectionWithContext<T>(
    eventName: string,
    promise: Promise<T>,
  ): Promise<T> {
    promise.catch((err: unknown) => {
      this.context.logger.error(
        `Unhandled rejection for event "${eventName}"`,
        {
          eventName,
          correlationId: this.context.correlationId,
  ...
  public async execute<TData, TResult>(
    ...
    if (!Value.Check(workflow.dataSchema, data)) {
      ...
      const errorDetails = errors
        ...
        .join(", ");
      throw new Error(
        `Workflow "${workflow.name}" data validation failed: ${errorDetails}`,
      );
    }
    ...
    if (!consumerEntry) {
      ...
      const definition = this.workflowCoordinator.getWorkflowDefinition(
        workflow.name,
      );
      if (!definition) {
        throw new Error(
      ...
      this.context.logger.warn(
        `Cannot execute workflow "${workflow.name}": no consumer registered. ` +
    ...
    this.executeConsumer(
      ...
      try {
        this.context.logger.error(
          `Workflow "${workflow.name}" execution failed`,
          { error },
        );
      } catch {
...
export class RequestBoundSocketEmitter implements SocketEmitter {
   ...
   */
  public emit<TData>(
    message: SocketMessageLike<TData>,
    topic: string,
    data: TData,
  ): Promise<void> {
