...
export class RequestPipeline {
  ...
  public createHandler(
    ...
    const contextFactory = new RequestContextFactory(
      appContext,
      sharedLoggerOptions,
    );

    // Pre-resolve guards at route registration time (singleton instances)
    const guards = hasGuards
      ? route.guards.map((G) => this.container.resolve(G))
      : [];

    // Pre-resolve interceptors at route registration time (singleton instances)
    const interceptors = hasInterceptors
      ? route.interceptors.map((I) => this.container.resolve(I))
      : [];

    // Pre-instantiate param validators at route registration time (no DI needed)
    const paramValidators = route.paramValidators
      ? new Map<string, ParamValidator>(
          [...route.paramValidators].map(([k, V]) => [k, new V()]),
        )
      : null;
      ...
      return (req: BunRequest): Promise<Response> => {
        ...
        const handleError = (error: unknown): Response => {
          ...
          return finalizeResponse(
            this.responseFactory.error(error, {
              correlationId,
              exposeDetails: false,
            }),
          );
        ...
        return runWithContext(
          { log: this.appLogger, correlationId, trace },
          () => {
            try {
        ...
        async () => {
          try {
            ...
            if (deferGuards && hasGuards) {
              ...
              ctx.setGuardRunner((_ctx, _options) =>
                this.runGuards(guards, _ctx),
              );
            } else if (hasGuards) {
  ...
  private prefixErrors(
    ...
    return errors.map((e) => {
      ...
      const normalizedPath = e.path.startsWith("/")
        ? e.path.slice(1).replace(/\//g, ".")
        : e.path;
      return {
        ...e,
        path: normalizedPath ? `${prefix}.${normalizedPath}` : prefix,
      };
    });
  }
  ...
  private validateParams(
    ...
    for (const [name, validator] of validators) {
      ...
      if (value === undefined || !validator.validate(value)) {
        errors.push({
          path: `params.${name}`,
          message: `Invalid value for path parameter '${name}'`,
        });
      }
    }
    return errors.length > 0
      ? this.responseFactory.validationError(errors)
      : null;
  }
