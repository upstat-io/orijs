...
> implements BaseContext {
  ...
  private _guardRunner:
    | ((ctx: any, options: Record<string, unknown>) => Promise<Response | null>)
    | null = null;
  ...
  public async guards(options: Record<string, unknown>): Promise<void> {
    if (!this._guardRunner) {
      throw new Error(
        "ctx.guards() requires deferGuards: true in the route schema options",
      );
    }
  ...
  get log(): Logger {
    if (this.cachedLogger === null) {
      const logger = new Logger("Request", this.loggerOptions).with({
  ...
  get events(): EventEmitter {
    if (this.cachedEvents === null) {
      ...
      if (!coordinator) {
        throw new Error(
          "Event system not configured. Register events with .event() before using ctx.events",
        );
      }
  ...
  public getValidatedParam(key: string): string {
    ...
    if (value.length > MAX_PARAM_LENGTH) {
      throw new Error(
        `Param '${key}' exceeds max length (${MAX_PARAM_LENGTH})`,
      );
    }
  ...
  public getValidatedUUID(key: string): string {
    ...
    for (let i = 0; i < UUID_LENGTH; i++) {
      ...
      if (!isHex) {
        throw new Error(
          `Invalid UUID format for param '${key}': invalid character`,
        );
      }
    }
