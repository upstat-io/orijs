...
export class Container {
   ...
   */
  public register<T extends Constructor>(
    service: T,
  ...
  private async resolveInternalAsync<T>(token: InjectionToken<T>): Promise<T> {
    ...
    try {
      ...
      const resolvedDeps = await Promise.all(
        deps.map((dep) => this.resolveInternalAsync(dep)),
      );
      const constructorResult = new service(...resolvedDeps);

      // Await if constructor returns Promise (async constructor pattern)
      const instance = this.isPromise(constructorResult)
        ? await constructorResult
        : constructorResult;
  ...
  private prepareResolution<T>(token: InjectionToken<T>): {
  ...
  private checkResolutionTimeout(token: InjectionToken): void {
    ...
    if (elapsed > this.resolutionTimeoutMs) {
      ...
      const resolvingChain = [...this.resolving]
        .map((s) => this.getTokenName(s))
        .join(" -> ");
      this.logger.warn(
        `Slow Service Resolution: ${elapsed}ms resolving ${tokenName}`,
        {
          elapsed,
          service: tokenName,
  ...
  private createCircularDependencyError(service: Constructor): Error {
    const chain = [...this.resolving]
      ...
      .join(" -> ");
    return new Error(
      `Circular dependency detected: ${chain} -> ${service.name}`,
    );
  }
   ...
   * ```
   */
  public registerWithTokenDeps<T extends Constructor>(
    service: T,
    deps: InjectionToken[],
  ): void {
    this.registry.set(service, deps as Constructor[]);
