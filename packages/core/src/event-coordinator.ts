...
export class EventCoordinator {
  ...
  private eventDefinitions: Map<string, EventDefinition<unknown, unknown>> =
    new Map();
  ...
  constructor(
    ...
    private readonly defaultProviderFactory: EventProviderFactory = () =>
      new InProcessEventProvider(),
  ) {}
  ...
  public setProvider(provider: EventProvider): void {
    if (this.eventProvider) {
      this.logger.warn(
        "Event provider already set, replacing with new provider",
      );
    }
  ...
  public registerEventDefinition<TPayload, TResponse>(
    ...
    if (this.eventDefinitions.has(eventName)) {
      throw new Error(
        `Duplicate event registration: "${eventName}" is already registered`,
      );
    }

    this.eventDefinitions.set(
      eventName,
      definition as EventDefinition<unknown, unknown>,
    );
  ...
  public addEventConsumer<TPayload, TResponse>(
    ...
    this.pendingConsumers.push({
      ...
      consumerClass: consumerClass as Constructor<
        IEventConsumer<unknown, unknown>
      >,
      deps,
    });
  }
  ...
  public registerConsumers(): void {
    ...
    if (!this.eventProvider) {
      if (this.pendingConsumers.length > 0 || this.eventDefinitions.size > 0) {
        this.logger.debug(
          "No event provider configured, using InProcessEventProvider",
        );
    ...
    for (const { definition, consumerClass, deps } of this.pendingConsumers) {
      ...
      const consumer =
        this.container.resolve<IEventConsumer<unknown, unknown>>(consumerClass);
      ...
      this.logger.info(
        `Event Consumer Registered -> [${eventName}] [${consumerClass.name}]`,
      );
    }
  ...
  private createValidatedHandler<TPayload, TResponse>(
    ...
    return async (message: EventMessage<TPayload>): Promise<TResponse> => {
      ...
      const ctx: EventContext<TPayload> = {
        ...
        prepareIdempotencyKeyRetirement: async (
          eventName: string,
          key: string,
        ): Promise<void> => {
          await this.eventProvider?.prepareIdempotencyKeyRetirement?.(
            eventName,
            key,
          );
        },
        finalizeIdempotencyKeyRetirement: async (
          eventName: string,
          key: string,
        ): Promise<void> => {
          await this.eventProvider?.finalizeIdempotencyKeyRetirement?.(
            eventName,
            key,
          );
        },
        hasSuccessfulIdempotencyKeyCompletionReceipt: async (
          eventName: string,
          key: string,
        ): Promise<boolean> => {
          return (
            (await this.eventProvider?.hasSuccessfulIdempotencyKeyCompletionReceipt?.(
              eventName,
              key,
            )) ?? false
          );
        },
        hasRetainedEvent: async (
          eventName: string,
          eventId: string,
        ): Promise<boolean> => {
          return (
            (await this.eventProvider?.hasRetainedEvent?.(
              eventName,
              eventId,
            )) ?? true
          );
        },
        isRetainedEventRetryable: async (
          eventName: string,
          eventId: string,
        ): Promise<boolean> => {
          return (
            (await this.eventProvider?.isRetainedEventRetryable?.(
              eventName,
              eventId,
            )) ?? true
          );
  ...
  public getEventDefinition(
    eventName: string,
  ): EventDefinition<unknown, unknown> | undefined {
  ...
  public async start(): Promise<void> {
    if (this.eventProvider) {
      ...
      if (consumerCount > 0) {
        ...
        this.logger.info(
          `Event Provider Started -> [${consumerCount} Consumers] [${defCount} Definitions] [${events}]`,
        );
      } else if (defCount > 0) {
        this.logger.info(
          `Event Provider Started -> [Emitter Only] [${defCount} Definitions]`,
        );
      } else {
