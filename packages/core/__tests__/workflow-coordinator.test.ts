...
describe("WorkflowCoordinator", () => {
  ...
  > {
    onComplete = async (
      _ctx: WorkflowContext<(typeof TestWorkflow)["_data"]>,
    ) => {
      return { success: true };
    };
  }
  ...
  const createMockProvider = (
    overrides: Partial<WorkflowProvider> = {},
  ): WorkflowProvider =>
    ({
      start: async () => {},
      stop: async () => {},
  ...
  describe("Workflow Definition Registration", () => {
    it("should register workflow definition", () => {
      ...
      expect(coordinator.getWorkflowDefinition("test-workflow")?.name).toBe(
        "test-workflow",
      );
    });
  ...
  describe("Worker Registration (CRITICAL - Consumer Availability)", () => {
    ...
    it("should have consumer ONLY for workflows with registered consumers", () => {
      ...
      class ConsumerWorkflowHandler implements IWorkflowConsumer<
        (typeof ConsumerWorkflow)["_data"],
        void
      > {
        onComplete = async (
          _ctx: WorkflowContext<(typeof ConsumerWorkflow)["_data"]>,
        ) => {};
      }
      ...
      coordinator.addWorkflowConsumer(
        ConsumerWorkflow,
      ...
      const consumerWorkflowConsumer =
        coordinator.getConsumer("consumer-workflow");
      expect(consumerWorkflowConsumer).toBeDefined();
      expect(consumerWorkflowConsumer?.consumer).toBeInstanceOf(
        ConsumerWorkflowHandler,
      );
    ...
    it("should allow executing workflows without having a consumer (executor-only pattern)", async () => {
      ...
      workerCoordinator.addWorkflowConsumer(
        TestWorkflow,
        TestWorkflowConsumer,
        [],
      );
      ...
      expect(
        executorCoordinator.getWorkflowDefinition("test-workflow"),
      ).toBeDefined();
      expect(executorCoordinator.getConsumer("test-workflow")).toBeUndefined();

      // Worker has both definition and consumer
      expect(
        workerCoordinator.getWorkflowDefinition("test-workflow"),
      ).toBeDefined();
  ...
  describe("IWorkflowConsumer with Steps (New Design)", () => {
    ...
    > {
      steps = {
        ...
        addTen: {
          execute: async (ctx: {
            ...
            const doubled =
              (ctx.results["double"] as { doubled: number } | undefined)
                ?.doubled ?? ctx.data.value;
      ...
      onComplete = async (
        ctx: WorkflowContext<(typeof SequentialWorkflow)["_data"]>,
      ) => {
        executionLog.push("onComplete");
        const addTenResult = ctx.results["addTen"] as
    ...
    it("uses the caller workflow id for registered consumer execution", async () => {
      ...
      expect(
        executionLog.filter((entry) => entry === "onComplete"),
      ).toHaveLength(1);
      await coordinator.stop();
    });
    ...
    it("should validate workflow result against schema", async () => {
      ...
      > {
        onComplete = async (
          _ctx: WorkflowContext<(typeof SimpleWorkflow)["_data"]>,
        ) => {
      ...
      await expect(
        executor.execute(SimpleWorkflow, { value: 5 }),
      ).rejects.toThrow(/result validation failed/i);
    ...
    const RollbackWorkflow = Workflow.define({
      ...
      s
        ...
        .sequential(
          s.step("step2", Type.Object({ processed: Type.Boolean() })),
        ),
    );
    ...
    it("should execute rollbacks in reverse order when a step fails", async () => {
      ...
      await expect(
        executor.execute(FailingWorkflow, { shouldFail: true }),
    ...
    it("should continue rollbacks even if one rollback fails", async () => {
      ...
      coordinator.addWorkflowConsumer(
        RollbackFailWorkflow,
        RollbackFailConsumer,
        [],
      );
      coordinator.registerConsumers();
      ...
      await expect(executor.execute(RollbackFailWorkflow, {})).rejects.toThrow(
        "Step 3 failed!",
      );
