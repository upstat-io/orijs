...
describe("AppContext", () => {
  ...
  describe("executeStartupHooks", () => {
    ...
    test("should fail fast on error", async () => {
      ...
      await expect(appContext.executeStartupHooks()).rejects.toThrow(
        "Startup failed",
      );
  ...
  describe("executeReadyHooks", () => {
    ...
    test("should fail fast on error", async () => {
      ...
      await expect(appContext.executeReadyHooks()).rejects.toThrow(
        "Ready failed",
      );
      expect(order).toEqual([1]);
    });
      ...
      test("should replace NullConfigProvider with provided provider", async () => {
        ...
        const mockProvider = {
          get: async (key: string) =>
            key === "TEST_KEY" ? "test_value" : undefined,
        ...
        expect(await appContext.config.getRequired("TEST_KEY")).toBe(
          "test_value",
        );
      });
    ...
    describe("serialization protection", () => {
      ...
      test("should redact config in custom inspect output", () => {
        ...
        expect(inspectOutput).toBe(
          "AppContext { phase: 'ready', config: [REDACTED] }",
        );
      ...
      test("should protect secrets even when provider has sensitive data", async () => {
        ...
        expect(await appContext.config.get("API_KEY")).toBe(
          "sk_live_secret123456789",
        );
  ...
  describe("socket", () => {
    describe("generic type parameter", () => {
      test("should provide typed socket when generic parameter is specified", () => {
        ...
        const appContext = new AppContext<CustomSocketEmitter>(
          logger,
          container,
        );
