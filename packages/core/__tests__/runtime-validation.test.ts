...
describe("Runtime Validation", () => {
  ...
  describe("Event Payload Validation", () => {
    ...
    it("should reject payload with missing required field", async () => {
      ...
      await expect(subscription.toPromise()).rejects.toThrow(
        /payload validation failed|email/i,
      );
    });
    ...
    it("should reject payload with invalid field type", async () => {
      ...
      await expect(subscription.toPromise()).rejects.toThrow(
        /payload validation failed|age/i,
      );
    });
  ...
  describe("Event Response Validation", () => {
    ...
    it("should accept valid response", async () => {
      ...
      // Should not throw
      await expect(
        provider.emit("response.event", { action: "test" }).toPromise(),
    ...
    it("should reject invalid response from consumer", async () => {
      ...
      await expect(subscription.toPromise()).rejects.toThrow(
        /response validation failed|count/i,
      );
    });
  ...
  describe("Workflow Data Validation", () => {
    ...
    it("should accept valid workflow data", async () => {
      ...
      > {
        onComplete = async (
          _ctx: WorkflowContext<(typeof StrictWorkflow)["_data"]>,
        ) => {
          return { processed: true };
        };
      }
      ...
      app = Ori.create()
        ...
        .consumer(StrictWorkflowConsumer);
      await app.listen(getPort());
  ...
  describe("Consumer Lifecycle Hooks", () => {
    ...
    it("should call onError hook on event processing failure", async () => {
      ...
      > {
        ...
        onError = async (
          _ctx: EventContext<(typeof LifecycleEvent)["_data"]>,
          error: Error,
        ) => {
          errorCalled = true;
