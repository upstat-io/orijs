...
describe("RequestContext", () => {
  ...
  describe("getValidatedParam", () => {
    ...
    test("should throw on missing param", () => {
      ...
      expect(() => ctx.getValidatedParam("missing")).toThrow(
        "Missing required param: missing",
      );
    });
    ...
    test("should throw on empty param", () => {
      ...
      expect(() => ctx.getValidatedParam("empty")).toThrow(
        "Missing required param: empty",
      );
    });
  ...
  describe("getValidatedUUID", () => {
    ...
    test("should throw on missing UUID param", () => {
      ...
      expect(() => ctx.getValidatedUUID("missing")).toThrow(
        "Missing required UUID param: missing",
      );
    });
    ...
    test("should throw on empty UUID param", () => {
      ...
      expect(() => ctx.getValidatedUUID("uuid")).toThrow(
        "Missing required UUID param: uuid",
      );
    });
    ...
    test("should throw on dashes in wrong positions", () => {
      const request = new Request("http://localhost/");
      const ctx = createTestContext(request, {
        id: "550e84-00e29b-41d4a-716-446655440000",
      });
  ...
  describe("socket", () => {
    describe("generic type parameter", () => {
      test("should accept TSocket generic parameter for type-safe socket access", () => {
        ...
        const ctx = new RequestContext<
          ...
          CustomSocketEmitter
        >(mockApp, request, {}, request.url, -1, defaultLoggerOptions);
    ...
    test("should return request-bound socket emitter with correlation ID", () => {
      ...
      expect(socket).toBeInstanceOf(RequestBoundSocketEmitter);
      expect((socket as RequestBoundSocketEmitter).correlationId).toBe(
...
describe("parseQuery", () => {
  ...
  test("should handle URL encoded values", () => {
    const url = new URL(
      "http://localhost/?message=hello%20world&special=%26%3D",
    );
