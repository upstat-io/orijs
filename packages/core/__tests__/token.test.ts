...
describe("Container with Tokens", () => {
  ...
  describe("registerInstance with Token", () => {
    ...
    test("should support multiple tokens of same type", () => {
      ...
      expect(container.resolve(HotCache)).not.toBe(
        container.resolve(ColdCache),
      );
    });
  });
  ...
  describe("validation with tokens", () => {
    test("should report missing token in validation", () => {
      ...
      try {
        ...
        container.registerWithTokenDeps(ServiceWithMissingToken, [
          MissingToken,
        ]);

        expect(() => container.validate()).toThrow(
          /depends on token 'MissingToken'/,
        );
      } finally {
