...
import {
  ...
  type CompiledRoute,
  type BunRequest,
} from "../src/controllers/request-pipeline.ts";
...
describe("RequestPipeline", () => {
  ...
  describe("guards", () => {
    ...
    test("should run guards in order", async () => {
      ...
      (mockContainer.resolve as ReturnType<typeof mock>).mockImplementation(
        (ctor: new () => unknown) => {
          return new ctor();
        },
      );
  ...
  describe("response headers", () => {
    ...
    test("should not modify response when no response headers set", async () => {
      const route = createRoute({
        guards: [AllowGuard],
        handler: async () =>
          new Response("OK", { headers: { "X-Existing": "value" } }),
      });
    ...
    test("should apply response headers alongside CORS headers", async () => {
      ...
      const handler = pipeline.createHandler(
        route,
        mockAppContext,
        {},
        corsHeaders,
  ...
  describe("debug logging", () => {
    ...
    test("should log route hit after guards on full path", async () => {
      ...
      const response = await handler(
        createMockRequest("POST", "http://localhost/api/users"),
      );
    ...
    test("should include correct method and path in debug log", async () => {
      ...
      const response = await handler(
        createMockRequest("DELETE", "http://localhost/api/monitors/123"),
      );
