...
export const BENCHMARK_PROFILE_ENV = "ORIJS_BENCHMARK_PROFILE";
export type BenchmarkProfile = "full" | "quick";

export function resolveBenchmarkProfile(
  value = process.env[BENCHMARK_PROFILE_ENV],
): BenchmarkProfile {
  ...
  throw new Error(`${BENCHMARK_PROFILE_ENV} must be "full" or "quick"`);
}

export function benchmarkCount(full: number, quick: number): number {
  return resolveBenchmarkProfile() === "quick" ? quick : full;
}
...
export async function runBenchmark(
  ...
  const {
    totalRequests = benchmarkCount(10000, 1000),
    concurrency = benchmarkCount(50, 10),
    warmupIterations = benchmarkCount(10, 2),
    headers = {},
  } = options;
  ...
  // Run benchmark
  console.log(
    `Running ${totalRequests} requests with concurrency ${concurrency}...`,
  );
...
export async function runScenarios(
  ...
  for (const scenario of scenarios) {
    ...
    if (method !== "GET" || body) {
      const result = await runBenchmarkWithOptions(
        scenario.url,
        fetchOptions,
        options,
      );
