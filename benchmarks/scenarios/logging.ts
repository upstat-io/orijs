...
import { benchmarkCount } from "../runner.ts";
...
const asyncIterations = benchmarkCount(50000, 2000);
...
console.log(
  `  Throughput: ${(asyncIterations / (asyncElapsed / 1000)).toFixed(0)} logs/sec\n`,
);
...
const syncIterations = benchmarkCount(10000, 1000);
...
console.log(
  `  Throughput: ${(syncIterations / (syncElapsed / 1000)).toFixed(0)} logs/sec\n`,
);
...
const errorIterations = benchmarkCount(100000, 5000);
...
console.log(
  `  Throughput: ${(errorIterations / (errorElapsed / 1000)).toFixed(0)} calls/sec`,
);
